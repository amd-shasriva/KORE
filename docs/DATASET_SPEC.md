...
This spec is directly implementable against the KORE codebase (`kore/data/*`, `kore/tasks/*`, `kore/verifier/*`) and the record schemas in `kore/data/schemas.py` (`RepairRecord`, `RankedGroupRecord`, `WinRecord`).

> **Agentic trajectory records.** The agentic tool-use stage adds `AgenticTrajectoryRecord` (defined in `kore/agent/schema.py`: `messages`, `tool_trace`, `best_kernel`, `best_reward`, `turns_to_best`, `success`, `reflections`, `phase_trace`). See [`kore/data/README.md`](../kore/data/README.md) and [`kore/agent/README.md`](../kore/agent/README.md) for that record type and how it is generated (`gen_agentic.py`, live GPU/teacher trajectories) or reconstructed CPU-only from already-verified repair/wins/groups records (`synth_agentic.py`, the default `--agentic synth` mode). Sections 1–4 below (coverage, edge cases, volumes, verification rigor) apply to `RepairRecord`/`RankedGroupRecord`/`WinRecord`.
...
1. **The baseline is the task's declared `comparison_baseline`, and there are two lanes.** The *vendor lane* — 106 of 1,334 tasks — is measured against the real serving op (AITER / hipBLASLt / rocBLAS / CK) on-box via `--impl reference`: 63 declare an `aiter_*` baseline and 4 a `hipblaslt_*` one in `task.yaml`, and at runtime `_genops._vendor_baseline` resolves a further 33 `gemm_fusion` tasks to hipBLASLt and the 6 `gen_{silu,gelu}_mul_*` gated activations to AITER. This is the lane that differentiates KORE from KernelBench/Kevin, and it is the lane a production claim may cite. The *breadth lane* — the remaining 92%, including all 1,052 generated `genb_*` tasks — is measured against torch: `torch.compile`-fused when `KORE_COMPILE_BASELINE=1`, eager otherwise.

   Two consequences follow, and both are load-bearing. First, a breadth-lane speedup says a kernel beats compiled PyTorch, not that it beats the state of practice — `WinRecord.baseline_type` records which lane produced each number and the two must never be pooled. Second, the **80** `genb_ssm_*` tasks (40 ops × 2 dtypes, generated by `kore/tasks/breadth/ssm_ext.py`) are graded against eager PyTorch references, and many of those are a Python `for t in range(L)` recurrence over the sequence axis — `L ∈ {256, 2047, 2048, 4096, 8192}` across their declared shapes. A fused Triton kernel beats that by orders of magnitude, so four-digit ratios from that family are real as measured and meaningless as claims. Records exceeding a credible-speedup threshold are flagged and excluded from exemplar selection and aggregate statistics.
...
Each edge case below is a *data requirement*: there must exist (a) at least one **verification shape/input** that exercises it, and (b) at least one **repair transition** (broken→fixed) whose failure is caused by mishandling it. Column "How to inject" maps to `kore/data/mutate.py` mutators (existing or to-add).
...
|---|---|---|---|
| R1 | VGPR spill | rocprofv3 `vgpr_count` through `kore/verifier/pmc.py` (CDNA4: 512 VGPR/lane, 160 KB LDS/CU) | tile too big; skills `reduce_vgpr_reload_lds`, `moe_register_aliasing` |
...
Every correctness/perf label MUST be produced by this pipeline. It is implemented across each task's `driver.py` (generated by `kore/tasks/_genops.py`: `_run_correctness`, the adversarial battery, the paired timing protocol) and `kore/env/kore_env.py` (`KoreEnv._run` / `evaluate`: replay cache, determinism re-check, post-timing re-verification, PMC). The `kore/verifier/test.py` and `kore/verifier/bench.py` modules an earlier revision named no longer exist — `kore/verifier/` is now only `pmc.py` plus `parsers/`:
...
- **Exact dedup:** `dedup_by_source_hash` (exists in `kore/data/build_datasets.py`) on the representative source per record.
...
2. Extend `kore/data/mutate.py` with mutators for N8 (`break_fp8_variant`), C1 (`break_block_m_to_64`), L-family (stride/layout), R2 (`break_lds_overflow`); wire families in `OP_FAMILY_MUTATORS`.
3. Add the anti-cheat static gate (Section 4.2) into `verifier/` and the metadata block (Section 4.5) into `schemas.py`.
4. Make multi-shape × ≥5-seed × held-out-shape × CV<3% the default in each task's `driver.py` (via `kore/tasks/_genops.py`) and in `kore/env/kore_env.py`.
