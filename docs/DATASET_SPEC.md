...
1. **The baseline is the task's declared `comparison_baseline`, and there are two lanes.** The *vendor lane* — roughly 100 of 1,334 tasks — is measured against the real serving op (AITER / hipBLASLt / rocBLAS / CK) on-box via `--impl reference`. This is the lane that differentiates KORE from KernelBench/Kevin, and it is the lane a production claim may cite. The *breadth lane* — the remaining ~92%, including all 1,052 generated `genb_*` tasks — is measured against torch: `torch.compile`-fused when `KORE_COMPILE_BASELINE=1`, eager otherwise.

   Two consequences follow, and both are load-bearing. First, a breadth-lane speedup says a kernel beats compiled PyTorch, not that it beats the state of practice — `WinRecord.baseline_type` records which lane produced each number and the two must never be pooled. Second, ~94 sequence/SSM tasks declare a baseline that is an eager Python `for t in range(L)` recurrence over 2,048–8,192 timesteps; a fused Triton kernel beats that by orders of magnitude, so four-digit ratios from that family are real as measured and meaningless as claims. Records exceeding a credible-speedup threshold are flagged and excluded from exemplar selection and aggregate statistics.

   The stated objective — match or beat the production vendor library — applies to the vendor lane. Closing the gap on the breadth lane means upgrading its baselines, not reinterpreting its numbers.
...
|---|---|---|---|
| R1 | VGPR spill | rocprofv3 `vgpr_count` through `verifier/pmc.py` (CDNA4: 512 VGPR/lane, 160 KB LDS/CU) | tile too big; skills `reduce_vgpr_reload_lds`, `moe_register_aliasing` |
...
| **KernelBook** (18,162 torch↔Triton) | https://huggingface.co/datasets/GPUMODE/KernelBook, pinned at revision `b76504d8` | Massive breadth of correct-by-construction torch→Triton pairs; idiomatic Triton | Inductor targets NVIDIA; **not** MFMA/gfx950-tuned; no fp8/MoE/MLA; no perf-vs-AITER labels | Use for **Stage-1 breadth only** (25% of SFT). Never use for perf labels. A local Inductor re-capture path was specified and never executed; the removed `data/synthetic.py` implemented it. |
