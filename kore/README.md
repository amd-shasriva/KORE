...
| [`reward`](reward/README.md) | The lexicographic anti-hack reward ladder, the physics residual-descent reward, and the policy-invariant roofline **shaping potential** (`whitebox.phi_potential`, `shaping.py`) | [→](reward/README.md) |
...
- **Policy-invariant densification (paradigm-v2).** The roofline attainment `ρ` is added as Ng-Harada-Russell potential-based shaping (`F = γ·Φ' − Φ`, `Φ = ρ`), so it densifies per-turn credit toward Speed-of-Light *without changing the optimal policy* - a formal anti-gaming guarantee that holds at any weight.
