...
- **`ToolExecutor.candidate_phi`** — the roofline potential `phi_potential(task, obs)` for that turn (`None` on any physics gap, a shaping boundary). It feeds GRPO's policy-invariant PBS credit toward the roofline (`kore.reward.shaping`, weight `physics_shaping_weight`). The online potential is the PMC-free `η = T_min / T_measured`; the counter-grounded named-residual refinement `ρ` in `kore.reward.whitebox` engages only when rocprofv3 counters are supplied. `ρ` is **not** a validated predictor — the roofline study's verdict is `INTEGRITY_ONLY` and no family authorizes empirical residual shaping (see [`docs/P0_RESULTS.md`](../../docs/P0_RESULTS.md)).
