...
On the active default path, `_env` scrubs inherited API keys, proxy controls, Slurm/SSH state, `LD_PRELOAD`, and user site from the child, and gives it a private `HOME` and `TMPDIR`. Two non-credential AMD variables (`AMD_NTID`, `AMD_LLM_GATEWAY_URL`) are not on the denylist and do reach the child. The Triton and inductor caches are **shared** across evaluations via `KORE_COMPILE_CACHE_DIR`, so the compile cache is inside the candidate boundary on this path. Only the external-broker path in [`kore/sandbox`](../sandbox/README.md) builds the environment from an allowlist and makes the compiler caches per-evaluation.
...
These named counters feed the roofline shaping potential. `kore.reward.whitebox.phi_potential` turns them into `Φ = ρ = T_min/(T_min + N)`, the counter-grounded named-residual attainment (`N = stall + occupancy-deficit`), which GRPO adds as an approximately policy-invariant potential-based-shaping term (`physics_shaping_weight`; the invariance is approximate under GRPO's std-normalized group-relative advantage — see [`kore/reward`](../reward/README.md)). The online potential is the PMC-free attainment `η = T_min/T_measured`; `ρ` is its counter-grounded refinement. **`ρ` is not validated.** An earlier revision of this page cited "R² ≈ 0.98 offline"; that figure is a shared-denominator artifact and the current adjudicator returns `INTEGRITY_ONLY` with all three checks failing, so no operator family authorizes empirical residual shaping (`docs/P0_RESULTS.md`). Potential-based shaping preserves the ordering of returns whether or not `Φ` is predictive, so the wiring is sound; the predictive claim is not.
