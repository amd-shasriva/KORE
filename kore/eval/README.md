...
> **Head-to-head vs Opus 4.8 is UNPROVEN (pending this eval).** `vs_opus.py` and the paired-significance machinery are implemented and wired, but the campaign eval stage currently emits only the **KORE-vs-seed** paired comparison (`eval/paired_seed_vs_kore.json`). A **KORE-vs-Opus-4.8** number has **not been produced** - no head-to-head result exists yet, and none should be claimed until this eval is run against a real Opus-4.8 policy. The harness makes the comparison *possible and correctly paired*; it does not make it *proven*.
