...
Live split: **1,501 train / 45 eval** of 1,546 registered tasks — 43 `near_probe` plus the
...
> **Conditions 4–6 are currently unreachable in practice.** All 1,546 tasks are `gfx950` and every
...
1,546 `task.yaml` files: **1,471 declare a `torch_*` baseline**, 63 declare AITER, 4 declare
hipBLASLt, and 8 declare something else. At runtime `_genops._vendor_baseline` additionally upgrades
59 more when `KORE_USE_VENDOR_BASELINE=1` (the default): 35 `gemm_fusion` tasks to hipBLASLt (via
`torch.matmul` / `torch._scaled_mm`), 8 gated activations to AITER, and 16 breadth MoE and
