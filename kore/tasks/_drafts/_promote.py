...
EXPECTED_PERF_ONLY = {"flash_attn_sliding_decode_bf16"}
...
def main():
    ...
    commons_done = set()
    for fam in FAMILIES:
        rep = os.path.join(DRAFTS, f"_report_{fam}.json")
        if not os.path.exists(rep):
            print(f"WARN: no report {rep}; skipping family {fam}")
            continue
        for r in json.load(open(rep)):
            tid = r["task"]
            src = os.path.join(DRAFTS, fam, tid)
            ...
            # scrub any stray build artifacts before the move
