"""Seed Triton bf16 (MQA/GQA) flash-attention *decode* kernel for gfx950 (CDNA4).

Exposes ``flash_attn_decode(q, k, v)`` with q ``[B,1,H,D]``, k/v ``[B,Skv,KV,D]``.
One program per (batch, q-head): loads the single query vector, streams the KV context
in BLOCK_N chunks with online softmax (max-subtraction + running rescale) and fp32
accumulation, bf16 IO. GQA/MQA maps each Q head to its shared KV head (kv_h =
h // (H//KV); KV==1 -> all heads share the single kv head). Boundary masking keeps it
correct for KV lengths not a multiple of the tile.
"""
