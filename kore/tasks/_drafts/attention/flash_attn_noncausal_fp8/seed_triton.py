"""Seed Triton fp8 NON-causal (GQA) flash attention for gfx950/CDNA4 (fp8 QKV -> bf16 out).

Exposes ``flash_attn(q, k, v, sq, sk, sv, causal=False)`` with q ``[B,S,H,D]`` fp8, k/v
``[B,S,KV,D]`` fp8, per-tensor fp32 scales sq/sk/sv. Loads fp8 in-register, folds the
dequant scales (``qk_scale = sq*sk/sqrt(D)`` into the QK product; ``v_scale = sv`` onto
the output), online-softmax with fp32 accumulation, bf16 IO. With causal=False every
query streams the FULL KV context. A correct seed the KORE policy optimizes against the
AITER FMHA bar (the fp8 kernel wins by moving ~half the QKV bytes of bf16).
"""
...
def flash_attn(q, k, v, sq, sk, sv, causal: bool = False):
    B, S, H, D = q.shape
    KV = k.shape[2]
