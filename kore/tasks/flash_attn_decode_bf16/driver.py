"""Driver for the bf16 (GQA) flash-attention decode task.

Correctness (no --bench-mode): candidate ``flash_attn_decode(q, k, v)`` vs exact
fp32 non-causal GQA SDPA oracle; prints SNR / allclose / max_diff. bf16 decode
attention gate is ~25 dB.
Bench (--bench-mode):
    --impl reference  -> AITER ``flash_attn_func`` (seq_q=1 decode bar)
...
def run_bench(shape, impl, warmup, iters) -> int:
    ...
    if impl == "reference":
        fn = lambda: aiter_flash_attn(q, k, v, causal=False)
    elif impl == "torch":
        fn = lambda: ref.attn_ref(q, k, v)
    else:
        cand = _load_candidate()
        fn = lambda: cand(q, k, v)
