"""Seed Triton bf16 NON-causal (GQA) flash-attention prefill for gfx950 (CDNA4).

Exposes ``flash_attn(q, k, v, causal=False)`` with q ``[B,S,H,D]``, k/v
``[B,S,KV,D]``. Online-softmax (max-subtraction + running rescale), fp32
accumulation, bf16 IO. With causal=False every query streams the FULL KV context.
Each Q head maps to its shared KV head (kv_h = h // (H//KV)). Boundary masking keeps
it correct for seqlens that are not a multiple of the tile. A correct, reasonably-
tuned seed the KORE policy optimizes against the AITER FMHA serving bar.
"""
...
def flash_attn(q: torch.Tensor, k: torch.Tensor, v: torch.Tensor, causal: bool = False):
    B, S, H, D = q.shape
    KV = k.shape[2]
    scale = 1.0 / (D ** 0.5)
