"""Seed Triton bf16 causal (GQA) flash-attention prefill for gfx942.

Exposes ``flash_attn(q, k, v, causal=True)`` with q ``[B,S,H,D]``, k/v
``[B,S,KV,D]``. Online-softmax (max-subtraction + running rescale), fp32
accumulation, bf16 IO. GQA is handled by mapping each Q head to its shared KV
head. Correct, reasonably-tuned baseline the KORE policy learns to optimize
against the AITER FMHA serving bar. Boundary masking makes it correct for
seqlens that are not a multiple of the tile.
"""
...
def _fa_kernel(
    ...
    sqb, sqs, sqh,            # q strides: batch, seq, head (last dim contiguous)
    skb, sks, skh,            # k strides
    svb, svs, svh,            # v strides
    sob, sos, soh,            # o strides
    S, scale,
