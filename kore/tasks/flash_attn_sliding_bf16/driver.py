"""Driver for bf16 sliding-window causal (GQA) flash attention.

Correctness (no --bench-mode): candidate ``flash_attn(q, k, v, window)`` vs the
exact fp32 sliding-window-causal SDPA oracle; prints SNR / allclose / max_diff
(bf16 attention gate ~25 dB, online-softmax accumulation).
Bench (--bench-mode):
    --impl reference  -> AITER dense causal ``flash_attn_func`` (the FMHA bar the
                         window kernel beats by skipping out-of-band key blocks)
...
def run_bench(shape, impl, warmup, iters) -> int:
    ...
    if impl == "reference":
        fn = lambda: aiter_flash_attn(q, k, v, causal=True)   # dense causal FMHA bar
    elif impl == "torch":
        fn = lambda: ref.attn_ref(q, k, v, window=window)
    else:
        cand = _load_candidate()
        fn = lambda: cand(q, k, v, window=window)
