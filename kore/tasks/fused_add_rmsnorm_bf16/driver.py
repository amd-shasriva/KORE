"""Driver for fused add-RMSNorm (KernelForge verifier contract).

Correctness (no --bench-mode): candidate ``fused_add_rmsnorm(x, res, w)`` ->
(y, added) vs exact torch-fp32 oracle for BOTH outputs; prints SNR/allclose/
max_diff (worst over y and the updated residual).
Bench (--bench-mode):
    --impl reference  -> AITER ``fused_add_rms_norm_cu`` (in-place, real bar)
...
from kore.tasks.aiter_ref import aiter_fused_add_rms_norm  # noqa: E402
...
def run_correctness(shape, mode) -> int:
    ...
    for s in seeds:
        x, residual, w = ref.get_inputs(shape, device=dev, seed=s)
        y_ref, added_ref = ref.fused_add_rmsnorm_ref(x, residual, w)
        try:
            y, added = fn(x, residual, w)
        except Exception as e:
            print("SNR: -999.00 dB"); print("allclose: False"); print("max_diff: inf")
            print(f"CANDIDATE_ERROR: {type(e).__name__}: {e}")
            return 0
        torch.cuda.synchronize()
        worst = min(worst, snr_db(y, y_ref), snr_db(added, added_ref))
        maxd = max(maxd, (y.float() - y_ref.float()).abs().max().item(),
                   (added.float() - added_ref.float()).abs().max().item())
        ok = ok and torch.allclose(y.float(), y_ref.float(), atol=1e-2, rtol=1e-2)
        ok = ok and torch.allclose(added.float(), added_ref.float(), atol=1e-2, rtol=1e-2)
...
def run_bench(shape, impl, warmup, iters) -> int:
    dev = "cuda"
    x, residual, w = ref.get_inputs(shape, device=dev, seed=0)
    if impl == "reference":
        # AITER op is in-place on (input, residual); operate on persistent
        # clones so we time the steady op cost (real serving usage).
        xb, rb = x.clone(), residual.clone()
        fn = lambda: aiter_fused_add_rms_norm(xb, rb, w, ref.EPS)
    elif impl == "torch":
        fn = lambda: ref.fused_add_rmsnorm_ref(x, residual, w)
    else:
        cand = _load_candidate()
        fn = lambda: cand(x, residual, w)
