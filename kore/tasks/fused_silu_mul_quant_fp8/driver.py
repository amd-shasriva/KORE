"""Driver for FUSED SiLU-gate-mul -> per-token fp8 quant (KernelForge contract).

Correctness (no --bench-mode): candidate ``quant(x) -> (xq, scale)``.
  * SNR  = SNR of the *dequantized* output ``xq*scale`` vs the TRUE fp32 silu_mul
           value (the real fidelity gate; fp8 e4m3 ~ low-20s dB).
...
from kore.tasks.aiter_ref import aiter_dynamic_per_token_quant, aiter_silu_and_mul  # noqa: E402
...
def run_correctness(shape, mode) -> int:
    ...
    for s in range(_num_correct_trials()):
        ...
        y_true = ref.silu_mul_ref(x)
        ref_xq, ref_scale = ref.fused_ref(x)
...
def run_bench(shape, impl, warmup, iters) -> int:
    ...
    if impl == "reference":
        def fn():
            y = aiter_silu_and_mul(x)
            return aiter_dynamic_per_token_quant(y)
    elif impl == "torch":
        fn = lambda: ref.fused_ref(x)
    else:
