"""Driver for the bf16 tanh-approx GELU task (KernelForge verifier contract).

Correctness (no --bench-mode): candidate ``gelu_tanh(x)`` vs exact torch-fp32
oracle; prints SNR / allclose / max_diff.
Bench (--bench-mode):
    --impl reference  -> ``F.gelu(approximate='tanh')`` == fused rocm elementwise
                         kernel (AITER ships only gated GELU)
...
from kore.tasks.aiter_ref import torch_gelu_tanh  # noqa: E402
...
def run_bench(shape, impl, warmup, iters) -> int:
    ...
    if impl == "reference":
        fn = lambda: torch_gelu_tanh(x)
    elif impl == "torch":
        fn = lambda: ref.gelu_tanh_ref(x)
    else:
