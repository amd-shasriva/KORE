"""Seed Triton fp8 (a8w8) per-token GEMM for gfx950 (MI350X/MI355X, CDNA4).
...
fp8 operands are up-converted in-register and accumulated in fp32; the per-row/per-col
scales are applied to the fp32 accumulator before the bf16 store (so a per-token scale
composes correctly with a per-channel scale). Stride-aware loads (handles a
non-contiguous / transposed XQ). A correct starter the KORE policy optimizes against the
AITER fp8 serving bar.
"""
