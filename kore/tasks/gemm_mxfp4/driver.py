"""Driver for the MXFP4 weight-only GEMM (KernelForge verifier contract).

Correctness (no --bench-mode): candidate ``matmul(a, w_packed, scale_e8m0) -> y``.
  * SNR  = SNR of y vs the fp32 oracle (dequantized mxfp4 weight matmul). The mxfp4
...
Bench (--bench-mode):
    --impl reference  -> materialize weight to bf16 + torch(hipBLASLt) matmul
                         (the memory-heavy bar the mxfp4 kernel beats)
