"""GENERATED seed Triton GEMM + fused epilogue for gemm_bias_sigmoid (fp32).

C = act(A @ B [+ bias]) in ONE kernel (fp32 accumulate, tl.float32 store). torch runs
...
def gemm_bias_sigmoid(a: torch.Tensor, b: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
    M, K = a.shape
    K2, N = b.shape
    ...
    _gemm_bias_sigmoid_kernel[grid](
        a, b, c, bias,
        M, N, K,
