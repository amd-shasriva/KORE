"""GENERATED seed Triton kernel for the mul_add_tanh (bf16) fusion.
...
@triton.jit
def _mul_add_tanh_kernel(a_ptr, b_ptr, c_ptr, o_ptr, sa, sb, sc, so, N, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
    col = tl.program_id(1)
    ...
    o = 2.0 * tl.sigmoid(2.0 * (a * b + c)) - 1.0
    tl.store(o_ptr + row * so + offs, o.to(tl.bfloat16), mask=mask)


def mul_add_tanh(a: torch.Tensor, b: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
    M, N = a.shape
    o = torch.empty_like(a)
    BLOCK_N = 1024
    grid = (M, triton.cdiv(N, BLOCK_N))
    _mul_add_tanh_kernel[grid](a, b, c, o, a.stride(0), b.stride(0), c.stride(0), o.stride(0), N,
