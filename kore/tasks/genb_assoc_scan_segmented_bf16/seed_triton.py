"""GENERATED breadth assoc_scan_segmented seed (bf16). Gated linear recurrence
...
parallel prefix scan the policy is expected to build). Inputs a[...,L], b[...,L]. tl.bfloat16 store."""
...
def _assoc_scan_segmented_kernel(a_ptr, b_ptr, h_ptr, L, srow):
    ...
    for i in range(0, L):
        ...
        h = av * h + bv
        tl.store(h_ptr + base + i, h.to(tl.bfloat16))
