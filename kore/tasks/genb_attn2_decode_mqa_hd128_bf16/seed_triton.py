"""GENERATED breadth attn2_decode_mqa_hd128 seed (bf16). Naive but correct fused flash-attention: one program per (query-block, batch*head) streams KV blocks with an online (max, sum) softmax (fp32 math); decode serving/inference variant, GQA via kv_head = head // group. The policy fuses/tiles it. tl.bfloat16 store."""
...
def attn2_decode_mqa_hd128(q, k, v):
