"""GENERATED breadth attn2_fp8_mqa_prefill_causal seed (fp8). Naive but correct fused flash-attention: one program per (query-block, batch*head) streams KV blocks with an online (max, sum) softmax (fp32 math); fp8 serving/inference variant, GQA via kv_head = head // group. The policy fuses/tiles it. tl.float8e4nv store."""
...
def attn2_fp8_mqa_prefill_causal(q, k, v):
    ...
    _attn2_fwd[grid](
        ...
        IS_CAUSAL=True, WINDOW=0, DILATION=1,
