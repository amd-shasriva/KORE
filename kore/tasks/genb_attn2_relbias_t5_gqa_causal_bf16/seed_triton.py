"""GENERATED breadth attn2_relbias_t5_gqa_causal seed (bf16). Naive but correct fused flash-attention: one program per (query-block, batch*head) streams KV blocks with an online (max, sum) softmax (fp32 math); relbias serving/inference variant, GQA via kv_head = head // group. The policy fuses/tiles it. tl.bfloat16 store."""
...
def _t5_rel_bias(bias_table, SQ, SK, num_buckets, max_distance):
...
def attn2_relbias_t5_gqa_causal(q, k, v, bias_table):
    q = q.contiguous(); k = k.contiguous(); v = v.contiguous()
    B, H, SQ, D = q.shape
    HKV = k.shape[1]
    SK = k.shape[2]
    bias = _t5_rel_bias(bias_table, SQ, SK, 32, 128)
