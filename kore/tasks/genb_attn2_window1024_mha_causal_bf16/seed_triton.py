"""GENERATED breadth attn2_window1024_mha_causal seed (bf16). Naive but correct fused flash-attention: one program per (query-block, batch*head) streams KV blocks with an online (max, sum) softmax (fp32 math); window serving/inference variant, GQA via kv_head = head // group. The policy fuses/tiles it. tl.bfloat16 store."""
...
def attn2_window1024_mha_causal(q, k, v):
