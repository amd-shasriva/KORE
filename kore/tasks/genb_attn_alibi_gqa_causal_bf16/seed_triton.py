"""GENERATED breadth attn_alibi_gqa_causal seed (bf16). Fused flash-attention forward: one program per (query-block, batch*head) streams KV blocks with an online (max, sum) softmax (fp32 math), GQA via kv_head = head // group; optional causal / sliding-window / ALiBi / soft-cap / sink. Naive but correct; the policy tunes the tiling. tl.bfloat16 store."""
...
def attn_alibi_gqa_causal(q, k, v, slopes):
    ...
    _attn_fwd[grid](
        q, k, v, o, slopes, q, sm_scale, H, HKV, SQ, SK,
        BLOCK_M=BLOCK_M, BLOCK_N=BLOCK_N, HEAD_DIM=D,
        IS_CAUSAL=True, WINDOW=0,
        USE_ALIBI=True, SOFTCAP=0.0, USE_SINK=False,
        num_warps=4)
    return o
