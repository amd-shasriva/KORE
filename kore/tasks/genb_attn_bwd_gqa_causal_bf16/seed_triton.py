"""GENERATED breadth attn_bwd_gqa_causal seed (bf16). Fused flash-attention BACKWARD (dQ/dK/dV) for causal attention: recompute the online-softmax log-sum-exp, then the standard flash dQ and dK/dV passes (GQA/MQA reduce over the kv group). Naive but correct; the policy fuses/tiles it. tl.bfloat16 store."""
...
def attn_bwd_gqa_causal(q, k, v, do):
