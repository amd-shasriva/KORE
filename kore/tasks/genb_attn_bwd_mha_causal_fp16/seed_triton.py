"""GENERATED breadth attn_bwd_mha_causal seed (fp16). Fused flash-attention BACKWARD (dQ/dK/dV) for causal attention: recompute the online-softmax log-sum-exp, then the standard flash dQ and dK/dV passes (GQA/MQA reduce over the kv group). Naive but correct; the policy fuses/tiles it. tl.float16 store."""
