"""GENERATED breadth attn_decode_mqa seed (bf16). Fused flash-attention forward: one program per (query-block, batch*head) streams KV blocks with an online (max, sum) softmax (fp32 math), GQA via kv_head = head // group; optional causal / sliding-window / ALiBi / soft-cap / sink. Naive but correct; the policy tunes the tiling. tl.bfloat16 store."""
