"""GENERATED breadth attn_softcap_mha_causal seed (bf16). Fused flash-attention forward: one program per (query-block, batch*head) streams KV blocks with an online (max, sum) softmax (fp32 math), GQA via kv_head = head // group; optional causal / sliding-window / ALiBi / soft-cap / sink. Naive but correct; the policy tunes the tiling. tl.bfloat16 store."""
...
def attn_softcap_mha_causal(q, k, v):
    ...
    _attn_fwd[grid](
        ...
        USE_ALIBI=False, SOFTCAP=30.0, USE_SINK=False,
        num_warps=4)
    return o
