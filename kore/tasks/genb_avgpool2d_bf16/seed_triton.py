"""GENERATED breadth avgpool2d seed (bf16) vs torch F.avg_pool2d(2).
2x2 stride-2 average pool: one program per (n, c, oh), mean over the 2x2 window
(divisor 4, no padding) across a vectorized output width, tl.bfloat16 store."""
...
def _avgpool2d_kernel(x_ptr, y_ptr, C, H, W, OH, OW,
    ...
    tl.store(y_ptr + y_off, (acc / 4.0).to(tl.bfloat16), mask=ow_mask)
