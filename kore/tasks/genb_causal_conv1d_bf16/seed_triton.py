"""GENERATED breadth causal_conv1d seed (bf16). Depthwise causal 1D conv (Mamba short conv).
...
tl.bfloat16 store. Naive per-time loop; the policy vectorizes over the time axis."""
...
def _causal_conv1d_kernel(x_ptr, w_ptr, b_ptr, y_ptr, D, L, K,
    ...
    for t in range(0, L):
        ...
        tl.store(y_ptr + base + t * sx_l, acc.to(tl.bfloat16))
