"""GENERATED breadth conv2d_nchw seed (fp16) vs torch F.conv2d.
...
the KORE policy optimizes against the torch baseline. tl.float16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _conv2d_nchw_kernel(x_ptr, w_ptr, b_ptr, y_ptr, Cin, H, W, Cout, OH, OW,
...
def conv2d_nchw(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
    N, Cin, H, W = x.shape
    ...
    grid = (N * Cout * OH,)
    _conv2d_nchw_kernel[grid](x, weight, bias, y, Cin, H, W, Cout, OH, OW,
