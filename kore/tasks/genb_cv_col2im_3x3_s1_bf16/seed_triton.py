"""GENERATED breadth cv_col2im_3x3_s1 seed (bf16). Naive col2im / fold (K=3, stride 1, square) vs torch F.fold; one program per (n, c, ih) overlap-adding the columns that map to it. The im2col adjoint. tl.bfloat16 store."""
...
def _cv_col2im_3x3_s1_kernel(c_ptr, y_ptr, C, H, W, OH, OW, ROWS,
    ...
    for kh in range(0, KH):
        ...
        for kw in range(0, KW):
            ...
            acc += cv
    tl.store(y_ptr + n * syn + c * syc + ih * syh + iw * syw, acc.to(tl.bfloat16), mask=iw_mask)
