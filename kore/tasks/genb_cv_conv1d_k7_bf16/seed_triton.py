"""GENERATED breadth cv_conv1d_k7 seed (bf16). Naive same conv1d (K=7, groups-generic) vs torch F.conv1d; one program per (n, cout, L-block), fp32 accumulate over (cin, k). The audio / SSM short conv - scan/fusion headroom. tl.bfloat16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _cv_conv1d_k7_kernel(x_ptr, w_ptr, b_ptr, y_ptr, Cin, L, Cout, OL, GIN, GOUT,
...
def cv_conv1d_k7(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
    N, Cin, L = x.shape
    Cout = weight.shape[0]
    GIN = weight.shape[1]
    GROUPS = Cin // GIN
    ...
    _cv_conv1d_k7_kernel[grid](x, weight, bias, y, Cin, L, Cout, OL, GIN, GOUT,
