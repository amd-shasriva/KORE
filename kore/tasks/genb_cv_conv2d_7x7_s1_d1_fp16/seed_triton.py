"""GENERATED breadth cv_conv2d_7x7_s1_d1 seed (fp16). Naive direct grouped conv2d (nchw, K=7 S=1 D=1) vs torch F.conv2d; one program per (n, cout, oh), fp32 accumulate over (cin, kh, kw), output width vectorized. Implicit-GEMM / channel-blocking headroom. tl.float16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _cv_conv2d_7x7_s1_d1_kernel(x_ptr, w_ptr, b_ptr, y_ptr, Cin, H, W, Cout, OH, OW, GIN, GOUT,
...
def cv_conv2d_7x7_s1_d1(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
    N, Cin, H, W = x.shape
    Cout = weight.shape[0]
    GIN = weight.shape[1]
    GROUPS = Cin // GIN
    ...
    grid = (N * Cout * OH,)
    _cv_conv2d_7x7_s1_d1_kernel[grid](x, weight, bias, y, Cin, H, W, Cout, OH, OW, GIN, GOUT,
