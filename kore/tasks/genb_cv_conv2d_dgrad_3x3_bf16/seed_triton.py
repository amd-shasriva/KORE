"""GENERATED breadth cv_conv2d_dgrad_3x3 seed (bf16). Naive conv2d dInput / dgrad (K=3, stride 1) vs torch.nn.grad.conv2d_input; one program per (n, cin, ih) accumulating grad_y * weight over (cout, kh, kw). The hard training backward kernel. tl.bfloat16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _cv_conv2d_dgrad_3x3_kernel(gy_ptr, w_ptr, gx_ptr, Cin, H, W, Cout, OH, OW,
    ...
    for co in range(0, Cout):
        for kh in range(0, KH):
            ...
            for kw in range(0, KW):
                ...
                acc += gv * wv
    tl.store(gx_ptr + n * sxn + cin * sxc + ih * sxh + iw * sxw, acc.to(tl.bfloat16), mask=iw_mask)


def cv_conv2d_dgrad_3x3(x: torch.Tensor, grad_y: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    N, Cin, H, W = x.shape
    Cout = weight.shape[0]
    ...
    grid = (N * Cin * H,)
    _cv_conv2d_dgrad_3x3_kernel[grid](grad_y, weight, gx, Cin, H, W, Cout, OH, OW,
