"""GENERATED breadth cv_conv2d_wgrad_1x1 seed (fp16). Naive conv2d dWeight / wgrad (K=1, stride 1) vs torch.nn.grad.conv2d_weight; one program per (cout, cin) reducing grad_y * x over (n, oh, ow) per tap. The hard training backward kernel. tl.float16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _cv_conv2d_wgrad_1x1_kernel(x_ptr, gy_ptr, gw_ptr, N, Cin, H, W, Cout, OH, OW,
    ...
    for kh in range(0, KH):
        for kw in range(0, KW):
            ...
            tl.store(gw_ptr + co * swo + cin * swc + kh * swh + kw * sww, acc.to(tl.float16))


def cv_conv2d_wgrad_1x1(x: torch.Tensor, grad_y: torch.Tensor, weight: torch.Tensor) -> torch.Tensor:
    N, Cin, H, W = x.shape
    Cout = weight.shape[0]
    ...
    grid = (Cout * Cin,)
    _cv_conv2d_wgrad_1x1_kernel[grid](x, grad_y, gw, N, Cin, H, W, Cout, OH, OW,
