"""GENERATED breadth cv_conv3d_3x3x3_s2 seed (fp16). Naive direct conv3d (K=3x3x3, S=2) vs torch F.conv3d; one program per (n, cout, od, oh), fp32 accumulate over (cin, kd, kh, kw), output width vectorized. Volumetric implicit-GEMM headroom. tl.float16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _cv_conv3d_3x3x3_s2_kernel(x_ptr, w_ptr, b_ptr, y_ptr, Cin, Din, H, W, Cout, OD, OH, OW,
    ...
    tl.store(y_ptr + n * syn + co * syc + od * syd + oh * syh + ow * syw,
             acc.to(tl.float16), mask=ow_mask)


def cv_conv3d_3x3x3_s2(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
    ...
    grid = (N * Cout * OD * OH,)
    _cv_conv3d_3x3x3_s2_kernel[grid](x, weight, bias, y, Cin, Din, H, W, Cout, OD, OH, OW,
