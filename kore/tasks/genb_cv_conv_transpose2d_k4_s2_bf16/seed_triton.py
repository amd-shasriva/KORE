"""GENERATED breadth cv_conv_transpose2d_k4_s2 seed (bf16). Naive transposed conv2d / deconv (K=4, S=2) vs torch F.conv_transpose2d; gather form (scatter inverse) - one program per (n, cout, oh) with a stride-modulo source map, fp32 accumulate. Upsampling headroom. tl.bfloat16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _cv_conv_transpose2d_k4_s2_kernel(x_ptr, w_ptr, b_ptr, y_ptr, Cin, H, W, Cout, OH, OW,
...
def cv_conv_transpose2d_k4_s2(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor) -> torch.Tensor:
    N, Cin, H, W = x.shape
    Cout = weight.shape[1]
    ...
    grid = (N * Cout * OH,)
    _cv_conv_transpose2d_k4_s2_kernel[grid](x, weight, bias, y, Cin, H, W, Cout, OH, OW,
