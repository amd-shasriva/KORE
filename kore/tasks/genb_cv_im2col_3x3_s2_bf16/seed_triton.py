"""GENERATED breadth cv_im2col_3x3_s2 seed (bf16). Naive im2col / unfold (K=3, S=2) vs torch F.unfold; one program per (n, cin*kh*kw row, L-block) gathering the padded patch. The implicit-GEMM lowering primitive. tl.bfloat16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _cv_im2col_3x3_s2_kernel(x_ptr, y_ptr, C, H, W, OH, OW, ROWS, LTOT,
    ...
    tl.store(y_ptr + n * syn + row * syr + l * syl, xv.to(tl.bfloat16), mask=l_mask)


def cv_im2col_3x3_s2(x: torch.Tensor) -> torch.Tensor:
    N, C, H, W = x.shape
    KH, KW = 3, 3
    ...
    _cv_im2col_3x3_s2_kernel[grid](x, y, C, H, W, OH, OW, ROWS, LTOT,
