"""GENERATED breadth cv_separable_conv2d_3x3 seed (fp16). Naive depthwise-separable conv2d = depthwise KxK (groups=C) then pointwise 1x1, as two chained kernels vs torch two-conv baseline; the policy fuses the two passes into one. tl.float16 store."""
...
def _cv_separable_conv2d_3x3_dw_kernel(x_ptr, w_ptr, t_ptr, C, H, W,
    ...
    for kh in range(0, K):
        ...
        for kw in range(0, K):
            ...
            acc += xv * wv
    tl.store(t_ptr + n * stn + c * stc + oh * sth + ow * stw, acc.to(tl.float16), mask=ow_mask)
