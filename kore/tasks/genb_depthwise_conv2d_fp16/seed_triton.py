"""GENERATED breadth depthwise_conv2d seed (fp16) vs torch F.conv2d(groups=C).
Naive depthwise conv: one program per (n, c, oh); each output channel convolves only
its own input channel over (kh, kw). fp32 accumulate, output width vectorized, tl.float16 store."""
