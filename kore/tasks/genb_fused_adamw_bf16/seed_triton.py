"""GENERATED breadth fused_adamw seed (bf16). One decoupled AdamW step, fused,
...
def _fused_adamw_kernel(p_ptr, g_ptr, m_ptr, v_ptr, numel,
    ...
    tl.store(m_ptr + offs, m.to(tl.bfloat16), mask=mask)
    tl.store(v_ptr + offs, v.to(tl.bfloat16), mask=mask)
