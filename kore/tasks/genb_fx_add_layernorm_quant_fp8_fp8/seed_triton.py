...
@triton.jit
def _fx_add_layernorm_quant_fp8_kernel(x_ptr, res_ptr, w_ptr, b_ptr, q_ptr, s_ptr, added_ptr, sm, N, eps, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
    base = row * sm
...
def fx_add_layernorm_quant_fp8(x, residual, weight, bias, eps: float = 1e-06):
    M, N = x.shape
    ...
    _fx_add_layernorm_quant_fp8_kernel[(M,)](x, residual, weight, bias, q, s, added, x.stride(0), N, eps,
