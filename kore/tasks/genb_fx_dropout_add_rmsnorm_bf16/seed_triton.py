...
@triton.jit
def _fx_dropout_add_rmsnorm_kernel(x_ptr, res_ptr, msk_ptr, w_ptr, y_ptr, added_ptr, sm, N, eps, inv_keep, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
    base = row * sm
...
def fx_dropout_add_rmsnorm(x, residual, mask, weight, eps: float = 1e-06, inv_keep: float = 1.1111111111111112):
    M, N = x.shape
    y = torch.empty_like(x)
    added = torch.empty_like(x)
    _fx_dropout_add_rmsnorm_kernel[(M,)](x, residual, mask, weight, y, added, x.stride(0), N, eps, inv_keep,
