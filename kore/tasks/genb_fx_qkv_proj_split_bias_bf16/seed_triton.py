...
@triton.jit
def _fx_qkv_proj_split_bias_mm(a_ptr, b_ptr, c_ptr, M, N, K, sam, sak, sbk, sbn, scm, scn,
...
def fx_qkv_proj_split_bias(x, weight, bias):
    M, K = x.shape
    N3 = weight.shape[1]
    BM, BN, BK = 64, 64, 32
    c = torch.empty((M, N3), device=x.device, dtype=x.dtype)
    grid = (triton.cdiv(M, BM), triton.cdiv(N3, BN))
    _fx_qkv_proj_split_bias_mm[grid](x, weight, c, M, N3, K, x.stride(0), x.stride(1), weight.stride(0), weight.stride(1), c.stride(0), c.stride(1), BM=BM, BN=BN, BK=BK)
    c = c + bias.reshape(1, N3)
    N = N3 // 3
