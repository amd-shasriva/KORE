...
@triton.jit
def _fx_swiglu_mlp_gateup_mm(a_ptr, b_ptr, c_ptr, M, N, K, sam, sak, sbk, sbn, scm, scn,
...
@triton.jit
def _fx_swiglu_mlp_gateup_gate(g_ptr, u_ptr, h_ptr, Ntot, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
    offs = pid * BLOCK + tl.arange(0, BLOCK)
    mask = offs < Ntot
    g = tl.load(g_ptr + offs, mask=mask, other=0.0).to(tl.float32)
    u = tl.load(u_ptr + offs, mask=mask, other=0.0).to(tl.float32)
    tl.store(h_ptr + offs, ((g * tl.sigmoid(g)) * u).to(tl.float16), mask=mask)


def fx_swiglu_mlp_gateup(x, wgu, wd):
    M, K = x.shape
    N2 = wgu.shape[1]
    N = N2 // 2
    BM, BN, BK = 64, 64, 32
    ...
    _fx_swiglu_mlp_gateup_mm[grid1](x, wgu, gu, M, N2, K, x.stride(0), x.stride(1), wgu.stride(0), wgu.stride(1), gu.stride(0), gu.stride(1), BM=BM, BN=BN, BK=BK)
    g = gu[:, 0:N].contiguous()
    u = gu[:, N:2 * N].contiguous()
    h = torch.empty((M, N), device=x.device, dtype=x.dtype)
    ntot = M * N
    _fx_swiglu_mlp_gateup_gate[(triton.cdiv(ntot, 1024),)](g, u, h, ntot, BLOCK=1024)
    out = torch.empty((M, K), device=x.device, dtype=x.dtype)
    grid2 = (triton.cdiv(M, BM), triton.cdiv(K, BN))
    _fx_swiglu_mlp_gateup_mm[grid2](h, wd, out, M, K, N, h.stride(0), h.stride(1), wd.stride(0), wd.stride(1), out.stride(0), out.stride(1), BM=BM, BN=BN, BK=BK)
    return out
