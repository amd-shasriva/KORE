"""GENERATED breadth GEMM seed: gemm_fp8_requant (fp8). Naive host dequant + a
...
def gemm_fp8_requant(aq, asc, wq, wsc, bias, osc):
    a = _dq_a8(aq, asc, "row").to(torch.bfloat16)
    w = _dq_w8(wq, wsc, "channel").to(torch.bfloat16)
    c = _mm_nt(a, w)
    y = c.float()
    y = y + bias.float().reshape(1, -1)
    y = (y / osc.float()).clamp(-448.0, 448.0).to(torch.float8_e4m3fn).float() * osc.float()
