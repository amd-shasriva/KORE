"""GENERATED breadth GEMM seed: gemm_int8_block128 (int8). Naive host dequant + a
...
def gemm_int8_block128(aq, asc, wq, wsc):
    a = _dq_a8(aq, asc, "block128").to(torch.bfloat16)
    w = _dq_w8(wq, wsc, "block128").to(torch.bfloat16)
    c = _mm_nt(a, w)
    y = c.float()
