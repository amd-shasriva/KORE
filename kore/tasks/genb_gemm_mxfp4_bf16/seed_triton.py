"""GENERATED breadth GEMM seed: gemm_mxfp4 (bf16). Naive host dequant + a
...
def gemm_mxfp4(apk, ae8, wpk, we8):
    a = _dq_mxfp4(apk, ae8).to(torch.bfloat16)
    w = _dq_mxfp4(wpk, we8).to(torch.bfloat16)
    c = _mm_nt(a, w)
    y = c.float()
