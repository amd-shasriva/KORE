"""GENERATED breadth GEMM seed: gemm_mxfp8 (bf16). Naive host dequant + a
...
def gemm_mxfp8(aq, ae8, wq, we8):
    a = _dq_mxfp8(aq, ae8).to(torch.bfloat16)
    w = _dq_mxfp8(wq, we8).to(torch.bfloat16)
    c = _mm_nt(a, w)
    y = c.float()
