"""GENERATED breadth GEMM seed: gemm_w4a16_bias_gelu (fp16). Naive host dequant + a
...
def gemm_w4a16_bias_gelu(a, wpk, wsc, wz, bias):
    a = a
    w = _dq_int4ga(wpk, wsc, wz, 128).to(torch.bfloat16)
    c = _mm_nt(a, w)
    y = c.float()
