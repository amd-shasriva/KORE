"""GENERATED breadth global_avgpool seed (fp16) vs torch global mean.
Global average over spatial dims: one program per (n, c) row reduces all H*W elements
in BLOCK-wide chunks (fp32 accumulate), output [N, C], tl.float16 store."""
...
def _global_avgpool_kernel(x_ptr, y_ptr, HW, BLOCK: tl.constexpr):
    ...
    tl.store(y_ptr + row, (acc / HW.to(tl.float32)).to(tl.float16))
