"""GENERATED breadth interpolate_nearest seed (fp16) vs torch F.interpolate.
2x nearest upsample: source index = dst // 2. One program per (n, c, oh), output width
vectorized gather, tl.float16 store."""
...
def _interpolate_nearest_kernel(x_ptr, y_ptr, C, H, W, OH, OW,
    ...
    tl.store(y_ptr + y_off, xv.to(tl.float16), mask=ow_mask)
