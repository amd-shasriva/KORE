"""GENERATED breadth linear_attention seed (bf16). Causal linear attention, phi=elu+1.
...
Naive sequential scan (the policy chunks/parallelizes it). tl.bfloat16 store."""
...
def _linear_attention_kernel(q_ptr, k_ptr, v_ptr, y_ptr, H, L, Dh,
    ...
    for l in range(0, L):
        ...
        tl.store(y_ptr + bh + l * s_l + e * s_d, y_le.to(tl.bfloat16))
