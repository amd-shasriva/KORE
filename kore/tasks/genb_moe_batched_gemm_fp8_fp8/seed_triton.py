"""GENERATED breadth MoE seed: moe_batched_gemm_fp8 (fp8).

fp8 a8w8 batched expert GEMM. Naive, COMPILING, CORRECT starting point: host-side routing/permute
...
def moe_batched_gemm_fp8(aq, bq, as_, bs):
    E, m, K = aq.shape
    N = bq.shape[1]
    out = torch.empty((E, m, N), device=aq.device, dtype=torch.bfloat16)
    for e in range(E):
        a = (aq[e].float() * as_[e].float()).to(torch.bfloat16)
        b = (bq[e].float() * bs[e].float()).to(torch.bfloat16)
        out[e] = _mm_nt(a, b)
    return out
