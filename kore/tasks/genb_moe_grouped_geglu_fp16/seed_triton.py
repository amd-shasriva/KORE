"""GENERATED breadth MoE seed: moe_grouped_geglu (fp16).

grouped gate/up GEMM fused with a GeGLU activation. Naive, COMPILING, CORRECT starting point: host-side routing/permute
...
def moe_grouped_geglu(hidden, w13, expert_ids):
    gu = _grouped_mm(hidden, w13, expert_ids)
    return _swiglu(gu, True).to(hidden.dtype)
