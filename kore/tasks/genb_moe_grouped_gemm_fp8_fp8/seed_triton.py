"""GENERATED breadth MoE seed: moe_grouped_gemm_fp8 (fp8).

fp8 a8w8 variable-M grouped expert GEMM. Naive, COMPILING, CORRECT starting point: host-side routing/permute
...
def moe_grouped_gemm_fp8(xq, wq, xs, ws, expert_ids):
    x = (xq.float() * xs.float()).to(torch.bfloat16)
    w = (wq.float() * ws.float()).to(torch.bfloat16)
    return _grouped_mm(x, w, expert_ids)
