"""GENERATED breadth MoE seed: moe_grouped_mlp_fp8 (fp8).

fp8 full grouped expert MLP. Naive, COMPILING, CORRECT starting point: host-side routing/permute
...
def moe_grouped_mlp_fp8(xq, w13q, w2q, xs, w13s, w2s, expert_ids):
    x = (xq.float() * xs.float()).to(torch.bfloat16)
    w13 = (w13q.float() * w13s.float()).to(torch.bfloat16)
    w2 = (w2q.float() * w2s.float()).to(torch.bfloat16)
    gu = _grouped_mm(x, w13, expert_ids)
    h = _swiglu(gu, False).to(torch.bfloat16)
    return _grouped_mm(h, w2, expert_ids)
