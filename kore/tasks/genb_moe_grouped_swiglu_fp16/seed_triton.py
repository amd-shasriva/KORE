"""GENERATED breadth MoE seed: moe_grouped_swiglu (fp16).

grouped gate/up GEMM fused with a SwiGLU activation. Naive, COMPILING, CORRECT starting point: host-side routing/permute
...
def moe_grouped_swiglu(hidden, w13, expert_ids):
    gu = _grouped_mm(hidden, w13, expert_ids)
    return _swiglu(gu, False).to(hidden.dtype)
