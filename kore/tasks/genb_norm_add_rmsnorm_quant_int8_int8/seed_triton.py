"""GENERATED breadth norm_add_rmsnorm_quant_int8 seed (int8) - fused add-residual + RMSNorm + per-row
int8 quant. Returns (q, scale, added) where added = x + residual (new residual)."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _norm_add_rmsnorm_quant_int8_kernel(x_ptr, res_ptr, w_ptr, q_ptr, s_ptr, added_ptr, sm, N, eps, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
    base = row * sm
...
def norm_add_rmsnorm_quant_int8(x: torch.Tensor, residual: torch.Tensor, weight: torch.Tensor, eps: float = 1e-06):
    M, N = x.shape
    ...
    _norm_add_rmsnorm_quant_int8_kernel[(M,)](x, residual, weight, q, s, added, x.stride(0), N, eps,
