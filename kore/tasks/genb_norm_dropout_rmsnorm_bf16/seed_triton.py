"""GENERATED breadth norm_dropout_rmsnorm seed (bf16) - RMSNorm then inverted dropout with a
...
def _norm_dropout_rmsnorm_kernel(x_ptr, w_ptr, msk_ptr, y_ptr, sm, N, eps, inv_keep, BLOCK_N: tl.constexpr):
    ...
    tl.store(y_ptr + row * sm + offs, (x * rstd * w * d * inv_keep).to(tl.bfloat16), mask=mask)
