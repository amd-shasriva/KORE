"""GENERATED breadth norm_layernorm_nobias_bwd seed (fp16) - LayerNorm (no bias) BACKWARD.
...
@triton.jit
def _norm_layernorm_nobias_bwd_kernel(x_ptr, w_ptr, dy_ptr, dx_ptr, dw_ptr, sm, N, eps, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
    base = row * sm
...
def norm_layernorm_nobias_bwd(x: torch.Tensor, weight: torch.Tensor, dy: torch.Tensor, eps: float = 1e-06):
    M, N = x.shape
    dx = torch.empty((M, N), device=x.device, dtype=torch.float32)
    dw = torch.zeros((N,), device=x.device, dtype=torch.float32)
    _norm_layernorm_nobias_bwd_kernel[(M,)](x, weight, dy, dx, dw, x.stride(0), N, eps,
