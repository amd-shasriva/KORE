"""GENERATED breadth norm_layernorm_quant_int8 seed (int8) - LayerNorm + per-row int8 output quant.
Returns (q, scale)."""
...
@triton.jit
def _norm_layernorm_quant_int8_kernel(x_ptr, w_ptr, b_ptr, q_ptr, s_ptr, sm, N, eps, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
...
def norm_layernorm_quant_int8(x: torch.Tensor, weight: torch.Tensor, bias: torch.Tensor, eps: float = 1e-06):
    M, N = x.shape
    q = torch.empty((M, N), device=x.device, dtype=torch.int8)
    s = torch.empty((M,), device=x.device, dtype=torch.float32)
    _norm_layernorm_quant_int8_kernel[(M,)](x, weight, bias, q, s, x.stride(0), N, eps,
