"""GENERATED breadth norm_rmsnorm_bwd_h4096 seed (bf16) - RMSNorm BACKWARD. Per-row dx =
...
@triton.jit
def _norm_rmsnorm_bwd_h4096_kernel(x_ptr, w_ptr, dy_ptr, dx_ptr, dw_ptr, sm, N, eps, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
    base = row * sm
...
def norm_rmsnorm_bwd_h4096(x: torch.Tensor, weight: torch.Tensor, dy: torch.Tensor, eps: float = 1e-06):
    M, N = x.shape
    dx = torch.empty((M, N), device=x.device, dtype=torch.float32)
    dw = torch.zeros((N,), device=x.device, dtype=torch.float32)
    _norm_rmsnorm_bwd_h4096_kernel[(M,)](x, weight, dy, dx, dw, x.stride(0), N, eps,
