"""GENERATED breadth norm_rmsnorm_quant_int8 seed (int8) - RMSNorm + per-row (per-token) int8
output quant. Returns (q, scale): scale = amax(normed)/127.0; q = normed/scale."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _norm_rmsnorm_quant_int8_kernel(x_ptr, w_ptr, q_ptr, s_ptr, sm, N, eps, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
...
def norm_rmsnorm_quant_int8(x: torch.Tensor, weight: torch.Tensor, eps: float = 1e-06):
    M, N = x.shape
    q = torch.empty((M, N), device=x.device, dtype=torch.int8)
    s = torch.empty((M,), device=x.device, dtype=torch.float32)
    _norm_rmsnorm_quant_int8_kernel[(M,)](x, weight, q, s, x.stride(0), N, eps,
