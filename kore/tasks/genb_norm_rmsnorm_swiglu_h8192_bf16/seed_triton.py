"""GENERATED breadth norm_rmsnorm_swiglu_h8192 seed (bf16) - RMSNorm(over 2H) then SwiGLU gate
silu(a)*b on the two halves. One program/row, fp32 reduction, tl.bfloat16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _norm_rmsnorm_swiglu_h8192_kernel(x_ptr, w_ptr, y_ptr, sxm, sym, TWOH, H, eps,
    ...
    tl.store(y_ptr + row * sym + offh, out.to(tl.bfloat16), mask=mh)


def norm_rmsnorm_swiglu_h8192(x: torch.Tensor, weight: torch.Tensor, eps: float = 1e-06) -> torch.Tensor:
    M, TWOH = x.shape
    H = TWOH // 2
    y = torch.empty((M, H), device=x.device, dtype=x.dtype)
    _norm_rmsnorm_swiglu_h8192_kernel[(M,)](x, weight, y, x.stride(0), y.stride(0), TWOH, H, eps,
