"""GENERATED breadth quant seed: qx_dequant_fp8_block2d (bf16). Naive host amax /
...
def qx_dequant_fp8_block2d(codes, scale):
    sf = scale.float().repeat_interleave(BLK, 0).repeat_interleave(BLK, 1)
    return _dq_map(codes, sf, torch.bfloat16)
