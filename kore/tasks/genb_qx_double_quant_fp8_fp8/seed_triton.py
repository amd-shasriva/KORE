"""GENERATED breadth quant seed: qx_double_quant_fp8 (fp8). Naive host amax /
...
def qx_double_quant_fp8(x):
    xf = x.float(); M, K = xf.shape
    xb = xf.reshape(M, K // BLK, BLK)
    sb = xb.abs().amax(dim=-1, keepdim=True).clamp(min=1e-12) / FP8_MAX
    inv = (1.0 / sb).expand(M, K // BLK, BLK).reshape(M, K)
    codes = _q_map(xf, inv, (-FP8_MAX), FP8_MAX, False, torch.float8_e4m3fn)
