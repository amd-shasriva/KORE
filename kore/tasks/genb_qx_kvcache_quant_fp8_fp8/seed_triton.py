...
def qx_kvcache_quant_fp8(k, v):
    ...
    kq = _q_map(k.float(), ks.expand_as(k.float()), (-FP8_MAX), FP8_MAX, False, torch.float8_e4m3fn)
    vs = v.float().abs().amax(dim=-1, keepdim=True).clamp(min=1e-12) / FP8_MAX
    vq = _q_map(v.float(), vs.expand_as(v.float()), (-FP8_MAX), FP8_MAX, False, torch.float8_e4m3fn)
