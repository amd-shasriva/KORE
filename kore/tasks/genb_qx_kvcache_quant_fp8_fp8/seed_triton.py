"""GENERATED breadth quant seed: qx_kvcache_quant_fp8 (fp8). Naive host amax /
...
def qx_kvcache_quant_fp8(k, v):
    ks = k.float().abs().amax(dim=-1, keepdim=True).clamp(min=1e-12) / FP8_MAX
    kq = _q_map(k.float(), (1.0 / ks).expand_as(k.float()), (-FP8_MAX), FP8_MAX, False, torch.float8_e4m3fn)
    vs = v.float().abs().amax(dim=-1, keepdim=True).clamp(min=1e-12) / FP8_MAX
    vq = _q_map(v.float(), (1.0 / vs).expand_as(v.float()), (-FP8_MAX), FP8_MAX, False, torch.float8_e4m3fn)
