"""GENERATED breadth quant seed: qx_kvcache_quant_int8 (int8). Naive host amax /
...
def qx_kvcache_quant_int8(k, v):
    ks = k.float().abs().amax(dim=-1, keepdim=True).clamp(min=1e-12) / INT8_MAX
    kq = _q_map(k.float(), (1.0 / ks).expand_as(k.float()), (-INT8_MAX), INT8_MAX, True, torch.int8)
    vs = v.float().abs().amax(dim=-1, keepdim=True).clamp(min=1e-12) / INT8_MAX
    vq = _q_map(v.float(), (1.0 / vs).expand_as(v.float()), (-INT8_MAX), INT8_MAX, True, torch.int8)
