"""GENERATED breadth quant seed: qx_quant_fp8_block2d (fp8). Naive host amax /
...
def qx_quant_fp8_block2d(x):
    xf = x.float()
    M, K = xf.shape
    xb = xf.reshape(M // BLK, BLK, K // BLK, BLK)
    sb = xb.abs().amax(dim=(1, 3), keepdim=True).clamp(min=1e-12) / FP8_MAX
