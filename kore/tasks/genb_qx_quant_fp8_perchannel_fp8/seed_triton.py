"""GENERATED breadth quant seed: qx_quant_fp8_perchannel (fp8). Naive host amax /
...
def qx_quant_fp8_perchannel(x):
    xf = x.float()
    amax = xf.abs().amax(dim=0, keepdim=True).clamp(min=1e-12)
    scale = amax / FP8_MAX
