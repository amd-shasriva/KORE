"""GENERATED breadth quant seed: qx_quant_int8_block2d (int8). Naive host amax /
...
def qx_quant_int8_block2d(x):
    xf = x.float()
    M, K = xf.shape
    xb = xf.reshape(M // BLK, BLK, K // BLK, BLK)
    sb = xb.abs().amax(dim=(1, 3), keepdim=True).clamp(min=1e-12) / INT8_MAX
