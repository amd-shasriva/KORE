"""GENERATED breadth quant seed: qx_smoothquant_fp8 (fp8). Naive host amax /
...
def qx_smoothquant_fp8(x, smooth):
    xf = x.float() / smooth.float().reshape(1, -1)
    amax = xf.abs().amax(dim=-1, keepdim=True).clamp(min=1e-12)
    scale = amax / FP8_MAX
    codes = _q_map(xf, (1.0 / scale).expand_as(xf), (-FP8_MAX), FP8_MAX, False, torch.float8_e4m3fn)
