"""GENERATED breadth red_bce_with_logits seed (fp16). logits[M,N], targets[M,N]
-> per-row mean binary-cross-entropy-with-logits, elementwise
max(x,0) - x*z + log1p(exp(-|x|)) (the numerically stable form). tl.float16 store."""
...
def _red_bce_with_logits_kernel(x_ptr, z_ptr, o_ptr, sx, sz, N, BLOCK_N: tl.constexpr):
    ...
    tl.store(o_ptr + row, (acc / N).to(tl.float16))
