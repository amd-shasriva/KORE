"""GENERATED breadth red_cosine_sim seed (fp16). a[M,N], b[M,N] -> per-row cosine
similarity <a,b>/(||a|| ||b||). Single streaming fp32 pass accumulates the dot and
both squared norms; denominator floored at 1e-08. tl.float16 store."""
...
def _red_cosine_sim_kernel(a_ptr, b_ptr, o_ptr, sa, sb, N, EPS, BLOCK_N: tl.constexpr):
    ...
    tl.store(o_ptr + row, (dot / denom).to(tl.float16))
