"""GENERATED breadth red_cross_entropy_bwd seed (fp32). logits[M,V]+targets[M] ->
...
at the target column. tl.float32 store."""
...
def _red_cross_entropy_bwd_kernel(x_ptr, t_ptr, o_ptr, sx, so, V, BLOCK: tl.constexpr):
    ...
    for start in range(0, V, BLOCK):
        ...
        tl.store(o_ptr + row * so + offs, grad.to(tl.float32), mask=mask)
