"""GENERATED breadth red_cross_entropy_zloss seed (fp16). logits[M,V]+targets[M]
-> per-row (logsumexp - logit[target]) + 0.0001 * logsumexp^2 (the PaLM log-Z^2
regularizer). Streaming max-subtracted lse (stable). tl.float16 store."""
...
def _red_cross_entropy_zloss_kernel(x_ptr, t_ptr, o_ptr, sx, V, COEF, BLOCK: tl.constexpr):
    ...
    tl.store(o_ptr + row, ((lse - xt) + COEF * lse * lse).to(tl.float16))
