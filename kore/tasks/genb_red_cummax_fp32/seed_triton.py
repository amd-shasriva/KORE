"""GENERATED breadth red_cummax seed (fp32). x[M,N] -> cumulative max over the
last dim. One program per row; a sequential fp32 running-max scan (naive but
correct; the policy replaces the serial loop with a parallel prefix scan). tl.float32."""
...
def _red_cummax_kernel(x_ptr, o_ptr, sx, so, N):
    ...
    for i in range(0, N):
        ...
        tl.store(o_ptr + row * so + i, run.to(tl.float32))
