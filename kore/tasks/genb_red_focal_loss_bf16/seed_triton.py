"""GENERATED breadth red_focal_loss seed (bf16). logits[M,V]+targets[M] -> the
multiclass focal loss -(1-pt)^2 * log(pt), pt = softmax(logits)[target] (gamma=2).
Streaming max-subtracted lse; log(pt) = logit[target] - lse (stable). tl.bfloat16 store."""
...
def _red_focal_loss_kernel(x_ptr, t_ptr, o_ptr, sx, V, BLOCK: tl.constexpr):
    ...
    pt = tl.exp(logpt)
    omp = 1.0 - pt
    tl.store(o_ptr + row, (-(omp * omp) * logpt).to(tl.bfloat16))
