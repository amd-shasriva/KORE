"""GENERATED breadth red_l2_normalize seed (fp16). x[M,N] -> a per-row rescaled output.
Two fp32 passes: pass 1 sums squares, pass 2 rescales x by the (rms/l2) factor. tl.float16."""
...
def _red_l2_normalize_kernel(x_ptr, o_ptr, sx, so, N, EPS, BLOCK_N: tl.constexpr):
    ...
    for start in range(0, N, BLOCK_N):
        ...
        mask = offs < N
        x = tl.load(x_ptr + row * sx + offs, mask=mask, other=0.0).to(tl.float32)
        tl.store(o_ptr + row * so + offs, (x * scale).to(tl.float16), mask=mask)
