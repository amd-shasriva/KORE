"""GENERATED breadth red_label_smoothing_ce seed (fp16). logits[M,V]+targets[M].
...
Matches F.cross_entropy(..., label_smoothing=eps). tl.float16 store."""
...
def _red_label_smoothing_ce_kernel(x_ptr, t_ptr, o_ptr, sx, V, EPS, BLOCK: tl.constexpr):
    ...
    nll = lse - xt
    smooth = lse - ssum / V
    tl.store(o_ptr + row, ((1.0 - EPS) * nll + EPS * smooth).to(tl.float16))
