"""GENERATED breadth red_layernorm seed (fp32). x[M,N] -> (x-mean)/sqrt(var+eps)
over the last dim (no affine), eps=1e-05. Three fp32 passes (mean, centered var,
write); the centered variance is numerically stable. tl.float32 store."""
...
def _red_layernorm_kernel(x_ptr, o_ptr, sx, so, N, EPS, BLOCK_N: tl.constexpr):
    ...
    for start in range(0, N, BLOCK_N):
        ...
        mask = offs < N
        x = tl.load(x_ptr + row * sx + offs, mask=mask, other=0.0).to(tl.float32)
        tl.store(o_ptr + row * so + offs, ((x - mean) * rstd).to(tl.float32), mask=mask)
