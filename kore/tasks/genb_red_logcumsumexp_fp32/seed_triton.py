"""GENERATED breadth red_logcumsumexp seed (fp32). x[M,N] -> cumulative
...
replaces the serial loop with a parallel prefix scan). tl.float32 store."""
...
def _red_logcumsumexp_kernel(x_ptr, o_ptr, sx, so, N):
    ...
    for i in range(0, N):
        ...
        run_m = new_m
        tl.store(o_ptr + row * so + i, (run_m + tl.log(run_s)).to(tl.float32))
