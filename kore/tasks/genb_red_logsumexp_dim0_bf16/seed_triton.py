"""GENERATED breadth red_logsumexp_dim0 seed (bf16). x[M,N] -> log-sum-exp over
dim 0 -> [N]. One program per column-block; streaming max-subtracted lse over the
M rows (fp32, stable). tl.bfloat16 store."""
...
def _red_logsumexp_dim0_kernel(x_ptr, o_ptr, sr, sc, M, N, BLOCK_N: tl.constexpr):
    ...
    for r in range(0, M):
        ...
        m = new_m
    out = m + tl.log(s)
    tl.store(o_ptr + cols, out.to(tl.bfloat16), mask=cmask)
