"""GENERATED breadth red_norm_linf seed (fp32). x[M,N] -> max_j |x_j| per row
(the L-infinity norm). Streaming fp32 running max of |x|. tl.float32 store."""
...
def _red_norm_linf_kernel(x_ptr, o_ptr, sx, N, BLOCK_N: tl.constexpr):
    ...
    tl.store(o_ptr + row, acc.to(tl.float32))
