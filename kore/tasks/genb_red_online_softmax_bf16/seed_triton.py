"""GENERATED breadth red_online_softmax seed (bf16). x[M,N] -> per-row softmax family
...
temperature. tl.bfloat16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _red_online_softmax_kernel(x_ptr, o_ptr, sx, so, N, INV_T, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
    m = -float("inf")
    s = 0.0
    ...
    for start in range(0, N, BLOCK_N):
        ...
        z = x - m
        out = tl.exp(z) / s
        tl.store(o_ptr + row * so + offs, out.to(tl.bfloat16), mask=mask)


def red_online_softmax(x: torch.Tensor) -> torch.Tensor:
    M, N = x.shape
    o = torch.empty_like(x)
    BLOCK_N = 1024 if N > 1024 else triton.next_power_of_2(N)
    _red_online_softmax_kernel[(M,)](x, o, x.stride(0), o.stride(0), N, 1.0,
