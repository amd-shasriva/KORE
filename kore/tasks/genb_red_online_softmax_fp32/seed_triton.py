"""GENERATED breadth red_online_softmax seed (fp32). x[M,N] -> per-row softmax family
...
def _red_online_softmax_kernel(x_ptr, o_ptr, sx, so, N, INV_T, BLOCK_N: tl.constexpr):
    ...
    for start in range(0, N, BLOCK_N):
        ...
        z = x - m
        out = tl.exp(z) / s
        tl.store(o_ptr + row * so + offs, out.to(tl.float32), mask=mask)
