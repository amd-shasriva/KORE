"""GENERATED breadth red_running_stats seed (fp16). x[M,N] -> (mean, biased var)
over dim 0 (batch) -> [N], [N] (batchnorm-style feature statistics). One program
per column-block; TWO fp32 passes over the rows (stable centered variance). tl.float16."""
...
def _red_running_stats_kernel(x_ptr, mean_ptr, var_ptr, sr, sc, M, N, BLOCK_N: tl.constexpr):
    ...
    for r in range(0, M):
        ...
        d = x - mean
        ss += d * d
    tl.store(mean_ptr + cols, mean.to(tl.float16), mask=cmask)
    tl.store(var_ptr + cols, (ss / M).to(tl.float16), mask=cmask)
