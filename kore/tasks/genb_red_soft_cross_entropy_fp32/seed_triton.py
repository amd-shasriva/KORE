"""GENERATED breadth red_soft_cross_entropy seed (fp32). logits[M,V], q[M,V] (a
distribution) -> -sum_j q_j log_softmax(logits)_j = lse*sum_j q_j - sum_j q_j x_j.
Pass 1 the streaming lse; pass 2 the weighted sums. tl.float32 store."""
...
def _red_soft_cross_entropy_kernel(x_ptr, q_ptr, o_ptr, sx, sq, V, BLOCK: tl.constexpr):
    ...
    tl.store(o_ptr + row, (lse * wsum - dot).to(tl.float32))
