"""GENERATED breadth red_softmax_dim0 seed (fp16). x[M,N] -> softmax over dim 0
...
def _red_softmax_dim0_kernel(x_ptr, o_ptr, sr, sc, M, N, BLOCK_N: tl.constexpr):
    ...
    for r in range(0, M):
        ...
        tl.store(o_ptr + r * sr + cols * sc, (tl.exp(x - m) / s).to(tl.float16), mask=cmask)
