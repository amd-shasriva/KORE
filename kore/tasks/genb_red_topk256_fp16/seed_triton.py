"""GENERATED breadth red_topk256 seed (fp16). x[M,N] -> the top-256 values per row,
descending. Naive but correct STREAMING threshold selection: 256 passes, each
...
answer is the top-256 values WITH MULTIPLICITY (``torch.sort(...)[:256]``), not the
256 largest DISTINCT values. Repeats are not an edge case here: fp16 has few
codes per octave, so thousands of the N values in a row collide exactly, and
skipping duplicates walks far down the distribution. tl.float16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _red_topk256_kernel(x_ptr, o_ptr, sx, so, N, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
    ...
    for i in range(0, 256):
        ...
        tl.store(o_ptr + row * so + i, cur.to(tl.float16))
        prev = cur


def red_topk256(x: torch.Tensor) -> torch.Tensor:
    M, N = x.shape
    o = torch.empty((M, 256), device=x.device, dtype=x.dtype)
    BLOCK_N = 1024
    _red_topk256_kernel[(M,)](x, o, x.stride(0), o.stride(0), N, BLOCK_N=BLOCK_N, num_warps=8)
    return o
