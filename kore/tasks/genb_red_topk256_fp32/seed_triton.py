"""GENERATED breadth red_topk256 seed (fp32). x[M,N] -> the top-256 values per row,
...
256 largest DISTINCT values. Repeats are not an edge case here: fp32 has few
codes per octave, so thousands of the N values in a row collide exactly, and
skipping duplicates walks far down the distribution. tl.float32 store."""
