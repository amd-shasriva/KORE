"""GENERATED breadth red_topk8 seed (fp16). x[M,N] -> the top-8 values per row,
descending. Naive but correct STREAMING threshold selection: 8 passes, each
...
answer is the top-8 values WITH MULTIPLICITY (``torch.sort(...)[:8]``), not the
8 largest DISTINCT values. Repeats are not an edge case here: fp16 has few
...
@triton.jit
def _red_topk8_kernel(x_ptr, o_ptr, sx, so, N, BLOCK_N: tl.constexpr):
    row = tl.program_id(0)
...
def red_topk8(x: torch.Tensor) -> torch.Tensor:
    M, N = x.shape
    o = torch.empty((M, 8), device=x.device, dtype=x.dtype)
    BLOCK_N = 1024
    _red_topk8_kernel[(M,)](x, o, x.stride(0), o.stride(0), N, BLOCK_N=BLOCK_N, num_warps=8)
    return o
