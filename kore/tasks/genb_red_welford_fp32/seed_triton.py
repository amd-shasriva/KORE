"""GENERATED breadth red_welford seed (fp32). x[M,N] -> (mean, biased var) per
row via a single-pass chunked Welford merge in fp32 (numerically stable running
mean + M2). tl.float32 store."""
...
def _red_welford_kernel(x_ptr, mean_ptr, var_ptr, sx, N, BLOCK_N: tl.constexpr):
    ...
    for start in range(0, N, BLOCK_N):
        ...
        count = new_count
    tl.store(mean_ptr + row, mean.to(tl.float32))
    tl.store(var_ptr + row, (m2 / count).to(tl.float32))
