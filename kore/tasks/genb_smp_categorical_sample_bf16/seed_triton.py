"""GENERATED breadth smp_categorical_sample seed (bf16). categorical inverse-CDF sample from softmax(logits) via supplied u. Naive but correct; the
...
def smp_categorical_sample(x: torch.Tensor, u: torch.Tensor) -> torch.Tensor:
    M, N = x.shape
