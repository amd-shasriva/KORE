"""GENERATED breadth smp_minp_mask seed (bf16). min-p mask: keep probs >= min_p * peak, then renormalize. Naive but correct; the
...
def smp_minp_mask(x: torch.Tensor) -> torch.Tensor:
    M, N = x.shape
    ...
    keep = probs >= 0.05 * probs.amax(-1, keepdim=True)
