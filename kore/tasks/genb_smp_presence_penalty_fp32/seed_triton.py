"""GENERATED breadth smp_presence_penalty seed (fp32). OpenAI presence penalty (subtract if seen). Naive but correct; the
...
def _elem_kernel(x_ptr, a_ptr, o_ptr, sx, sa, so, N, BLOCK_N: tl.constexpr):
    ...
    o = x - 0.5 * tl.where(a > 0.0, 1.0, 0.0)
    tl.store(o_ptr + row * so + offs, o.to(tl.float32), mask=mask)


def smp_presence_penalty(x: torch.Tensor, counts: torch.Tensor) -> torch.Tensor:
    M, N = x.shape
