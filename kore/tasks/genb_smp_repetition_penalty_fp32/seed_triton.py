"""GENERATED breadth smp_repetition_penalty seed (fp32). CTRL repetition penalty (sign-aware, on seen tokens). Naive but correct; the
...
def _elem_kernel(x_ptr, a_ptr, o_ptr, sx, sa, so, N, BLOCK_N: tl.constexpr):
    ...
    o = tl.where(a > 0.0, tl.where(x > 0.0, x / 1.2, x * 1.2), x)
    tl.store(o_ptr + row * so + offs, o.to(tl.float32), mask=mask)


def smp_repetition_penalty(x: torch.Tensor, seen: torch.Tensor) -> torch.Tensor:
    M, N = x.shape
    o = torch.empty_like(x)
    BLOCK_N = 1024
    grid = (M, triton.cdiv(N, BLOCK_N))
    _elem_kernel[grid](x, seen, o, x.stride(0), seen.stride(0), o.stride(0), N, BLOCK_N=BLOCK_N, num_warps=4)
    return o
