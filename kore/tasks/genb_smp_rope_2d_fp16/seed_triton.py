"""GENERATED breadth smp_rope_2d seed (fp16). 2D RoPE: two head-dim halves rotated by two position coordinates. Naive but correct; the
...
def smp_rope_2d(x: torch.Tensor, pos_h: torch.Tensor, pos_w: torch.Tensor) -> torch.Tensor:
    ...
    oh = torch.empty((M, Dh), device=device, dtype=torch.float16)
    ow = torch.empty((M, Dh), device=device, dtype=torch.float16)
