"""GENERATED breadth smp_rope_dynamic_ntk seed (fp16). dynamic-NTK RoPE (base grows with sequence length). Naive but correct; the
...
def smp_rope_dynamic_ntk(x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
    xf = x.float().contiguous()
    M, D = xf.shape
    half = D // 2
    device = xf.device
    i = torch.arange(half, device=device, dtype=torch.float32)
    bf = (4.0 * 16384 / 4096) - (4.0 - 1)
    theta = 10000.0 * (bf ** (D / (D - 2)))
    inv = theta ** (-(2.0 * i) / D)
    mscale = 1.0
