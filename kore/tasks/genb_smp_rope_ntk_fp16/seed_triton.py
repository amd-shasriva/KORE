"""GENERATED breadth smp_rope_ntk seed (fp16). static NTK-aware scaled RoPE. Naive but correct; the
...
def smp_rope_ntk(x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
    xf = x.float().contiguous()
    M, D = xf.shape
    half = D // 2
    device = xf.device
    i = torch.arange(half, device=device, dtype=torch.float32)
    theta = 10000.0 * (4.0 ** (D / (D - 2)))
    inv = theta ** (-(2.0 * i) / D)
    mscale = 1.0
