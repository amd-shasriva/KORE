"""GENERATED breadth smp_rope_partial seed (fp32). partial rotary: rotate the first rot_pct of head_dim, pass the rest. Naive but correct; the
...
def smp_rope_partial(x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
    ...
    o = xf.to(torch.float32).clone()
    xr = xf[:, :rot].contiguous()
    orot = torch.empty((M, rot), device=device, dtype=torch.float32)
