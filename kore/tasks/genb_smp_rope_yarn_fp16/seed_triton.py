"""GENERATED breadth smp_rope_yarn seed (fp16). YaRN RoPE (ramped interp/extrapolation + attention mscale). Naive but correct; the
...
def _rope_kernel(x_ptr, c_ptr, s_ptr, o_ptr, sx, sc, ss, so, HALF, BLOCK_H: tl.constexpr):
    ...
    tl.store(o_ptr + row * so + offs, (x1 * c - x2 * s).to(tl.float16), mask=mask)
    tl.store(o_ptr + row * so + HALF + offs, (x2 * c + x1 * s).to(tl.float16), mask=mask)


def smp_rope_yarn(x: torch.Tensor, pos: torch.Tensor) -> torch.Tensor:
    ...
    import math
    freq = 10000.0 ** ((2.0 * i) / D)
    inv_extra = 1.0 / freq
    inv_inter = 1.0 / (4.0 * freq)
    def corr(nr):
        return (D * math.log(4096 / (nr * 2.0 * math.pi))) / (2.0 * math.log(10000.0))
    low = max(math.floor(corr(32.0)), 0)
    high = min(math.ceil(corr(1.0)), half - 1)
    if high == low:
        high = low + 0.001
    ...
    o = torch.empty((M, D), device=device, dtype=torch.float16)
