"""GENERATED breadth smp_spec_accept seed (fp16). rejection-sampling accept: u <= min(1, p_target[d]/q_draft[d]). Naive but correct; the
...
def _acc_kernel(a_ptr, u_ptr, o_ptr, M, BLOCK: tl.constexpr):
    ...
    tl.store(o_ptr + offs, o.to(tl.float16), mask=mask)
...
def smp_spec_accept(q: torch.Tensor, p: torch.Tensor, d: torch.Tensor, u: torch.Tensor) -> torch.Tensor:
    ...
    o = torch.empty((M,), device=q.device, dtype=torch.float16)
    BLOCK = 256
