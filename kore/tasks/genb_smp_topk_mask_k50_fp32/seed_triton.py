"""GENERATED breadth smp_topk_mask_k50 seed (fp32). keep the top-50 logits per row, mask the rest to -inf. Naive but correct; the
...
def smp_topk_mask_k50(x: torch.Tensor) -> torch.Tensor:
    M, N = x.shape
    K = 50
