"""GENERATED breadth smp_tree_attn_mask seed (bf16). tree-attention mask from a parent table (node attends to its ancestors). Naive but correct; the
...
def _copy_kernel(x_ptr, o_ptr, sx, so, N, BLOCK_N: tl.constexpr):
    ...
    tl.store(o_ptr + row * so + offs, x.to(tl.bfloat16), mask=mask)
...
def smp_tree_attn_mask(parent: torch.Tensor) -> torch.Tensor:
    ...
    o = torch.empty((M * T, T), device=parent.device, dtype=torch.bfloat16)
