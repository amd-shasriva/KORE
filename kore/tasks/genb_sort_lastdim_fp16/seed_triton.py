"""GENERATED breadth row-sort seed (fp16). x[M,N] -> ascending sorted rows.
...
starting point; the teacher is expected to replace it with a bitonic sort. tl.float16."""
...
def _sort_lastdim_kernel(x_ptr, o_ptr, sx, so, N, BLOCK_N: tl.constexpr):
    ...
    for i in range(0, N):
        ...
        tl.store(o_ptr + row * so + i, v.to(tl.float16))
