"""GENERATED breadth ssm_gated_retention_c128 seed (fp16). Gated retention (data-dependent SCALAR
...
s = a*s + k_l*v_l[e]; y_l[e]=sum_d q_l[d]*s[d]. tl.float16 store."""
from __future__ import annotations
import torch, triton, triton.language as tl


@triton.jit
def _ssm_gated_retention_c128_kernel(q_ptr, k_ptr, v_ptr, g_ptr, y_ptr, H, L, Dh, s_b, s_h, s_l, s_d,
...
def ssm_gated_retention_c128(q, k, v, gl):
    B, H, L, Dh = q.shape
    q = q.contiguous(); k = k.contiguous(); v = v.contiguous(); gl = gl.contiguous()
    y = torch.empty_like(v)
    DB = triton.next_power_of_2(Dh)
    _ssm_gated_retention_c128_kernel[(B * H * Dh,)](q, k, v, gl, y, H, L, Dh,
