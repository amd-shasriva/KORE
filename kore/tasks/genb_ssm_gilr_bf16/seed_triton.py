"""GENERATED breadth ssm_gilr seed (bf16). GILR gated impulse linear recurrent over
the last dim: h_t = f_t h_{t-1} + i_t z_t, f=sigmoid(f_l), i=sigmoid(i_l). One
program per flattened row; sequential fp32 scan. tl.bfloat16 store."""
...
def _ssm_gilr_kernel(f_ptr, i_ptr, z_ptr, h_ptr, L, srow):
    ...
    for t in range(0, L):
        ...
        h = f * h + ii * z
        tl.store(h_ptr + base + t, h.to(tl.bfloat16))
