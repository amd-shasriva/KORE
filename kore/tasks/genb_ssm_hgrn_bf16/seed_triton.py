"""GENERATED breadth ssm_hgrn seed (bf16). HGRN gated linear RNN over the last dim:
h_t = f_t h_{t-1} + (1-f_t) g_t, f=sigmoid(f_l). One program per flattened row;
sequential fp32 scan (the policy builds the parallel prefix scan). tl.bfloat16 store."""
