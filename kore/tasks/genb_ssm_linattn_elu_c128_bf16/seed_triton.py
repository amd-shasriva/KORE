"""GENERATED breadth ssm_linattn_elu_c128 seed. Causal linear attention, feature map elu
...
@triton.jit
def _ssm_linattn_elu_c128_kernel(q_ptr, k_ptr, v_ptr, y_ptr, H, L, Dh, s_b, s_h, s_l, s_d,
                 DB: tl.constexpr):
    pid = tl.program_id(0)
    e = pid % Dh
    tmp = pid // Dh
    ...
    for l in range(0, L):
        ...
        phik = tl.where(krow > 0.0, krow + 1.0, tl.exp(krow))
        ...
        phiq = tl.where(qrow > 0.0, qrow + 1.0, tl.exp(qrow))
...
def ssm_linattn_elu_c128(q, k, v):
    B, H, L, Dh = q.shape
    q = q.contiguous(); k = k.contiguous(); v = v.contiguous()
    y = torch.empty_like(v)
    DB = triton.next_power_of_2(Dh)
    _ssm_linattn_elu_c128_kernel[(B * H * Dh,)](q, k, v, y, H, L, Dh,
