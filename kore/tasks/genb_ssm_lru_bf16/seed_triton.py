"""GENERATED breadth ssm_lru seed (bf16). Linear Recurrent Unit: complex
...
via an associative complex scan). lam's (lr, li) are precomputed. tl.bfloat16 store."""
...
def _ssm_lru_kernel(x_ptr, lr_ptr, li_ptr, y_ptr, D, L, srow, sl):
    ...
    for i in range(0, L):
        ...
        hr = hrn
        hi = hin
        tl.store(y_ptr + base + i * sl + 0, hr.to(tl.bfloat16))
        tl.store(y_ptr + base + i * sl + 1, hi.to(tl.bfloat16))
