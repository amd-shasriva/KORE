"""GENERATED breadth ssm_retnet_multiscale seed. RetNet/lightning retention (FIXED per-head decay
...
@triton.jit
def _ssm_retnet_multiscale_kernel(q_ptr, k_ptr, v_ptr, gamma_ptr, y_ptr, H, L, Dh, s_b, s_h, s_l, s_d,
                 DB: tl.constexpr):
    pid = tl.program_id(0)
    e = pid % Dh
    tmp = pid // Dh
...
def ssm_retnet_multiscale(q, k, v):
    B, H, L, Dh = q.shape
    gamma = torch.tensor([0.96875, 0.984375, 0.9921875, 0.99609375, 0.998046875, 0.9990234375, 0.99951171875, 0.999755859375], dtype=torch.float32, device=q.device)
    q = q.contiguous(); k = k.contiguous(); v = v.contiguous()
    y = torch.empty_like(v)
    DB = triton.next_power_of_2(Dh)
    _ssm_retnet_multiscale_kernel[(B * H * Dh,)](q, k, v, gamma, y, H, L, Dh,
