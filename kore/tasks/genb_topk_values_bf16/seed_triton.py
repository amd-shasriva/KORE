"""GENERATED breadth top-k values seed (bf16). x[M,N] -> top-k values per row.
...
VALUES (descending, ties are value-identical). tl.bfloat16 store."""
...
def _topk_values_kernel(x_ptr, o_ptr, sx, so, N, K, BLOCK_N: tl.constexpr):
    ...
    for k in range(0, K):
        ...
        tl.store(o_ptr + row * so + k, v.to(tl.bfloat16))
