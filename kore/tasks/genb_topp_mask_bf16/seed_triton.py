"""GENERATED breadth nucleus (top-p) seed (bf16). logits[M,N] -> renormalized
...
partial seed the teacher is expected to replace with a real top-p. tl.bfloat16 store."""
...
def _topp_mask_kernel(x_ptr, o_ptr, sx, so, N, P, BLOCK_N: tl.constexpr):
    ...
    tl.store(o_ptr + row * so + offs, (kp / denom).to(tl.bfloat16), mask=mask)
