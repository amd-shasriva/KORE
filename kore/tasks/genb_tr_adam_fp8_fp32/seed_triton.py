"""GENERATED breadth tr_adam_fp8 seed. Blockwise fp8-quantized Adam states: dequant (torch) -> fused fp32 Adam update in a Triton kernel -> requant (torch). Fusing the de/requant into the Triton kernel is the target."""
from __future__ import annotations
import torch, triton, triton.language as tl

_QB, _QMAX, _QDT = 128, 448.0, torch.float8_e4m3fn
...
def _quant(x):
    ...
    q = (xb / scale[:, None]).to(_QDT).reshape(-1)[:n].reshape(x.shape)
    return q, scale


@triton.jit
def _tr_adam_fp8_kernel(p_ptr, g_ptr, m_ptr, v_ptr, mo_ptr, vo_ptr, numel, lr, weight_decay, b1, b2, eps, step_size, bc2sqrt, BLOCK: tl.constexpr):
    pid = tl.program_id(0)
...
def tr_adam_fp8(param, grad, q_exp_avg, s_exp_avg, q_exp_avg_sq, s_exp_avg_sq, lr, b1, b2, eps, weight_decay, step):
    ...
    _tr_adam_fp8_kernel[grid](param, grad, m, v, mo, vo, numel, lr, weight_decay, b1, b2, eps, step_size, bc2sqrt, BLOCK=BLOCK, num_warps=4)
