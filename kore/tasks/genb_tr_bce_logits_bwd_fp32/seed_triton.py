"""GENERATED breadth tr_bce_logits_bwd seed. Triton computes both the
...
def _elem_loss_kernel(x_ptr, y_ptr, part_ptr, grad_ptr, numel,
    ...
    tl.store(grad_ptr + offs, grad.to(tl.float32), mask=mask)
...
def tr_bce_logits_bwd(inp, target):
    ...
    _elem_loss_kernel[(n_parts,)](
        x, y, parts, grad, numel, KIND=0, BLOCK=BLOCK, num_warps=4)
