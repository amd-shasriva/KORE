"""GENERATED breadth tr_reverse_kl_distill_bwd seed. Triton computes the fp32 row
    ...
    for start in range(0, V, BLOCK):
        ...
        if DISTILL:
            ...
            if KIND == 1:
                ...
                grad = (ps - onehot) / M
        tl.store(grad_ptr + base + offs, grad.to(tl.bfloat16), mask=mask)
...
def tr_reverse_kl_distill_bwd(student, teacher):
    x = student.contiguous()
    aux = teacher.contiguous()
    M, V = x.shape
    ...
    _loss_rows_kernel[(M,)](
        x, aux, rows, grad, M, V, KIND=7, DISTILL=True,
