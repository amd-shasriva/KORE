"""GENERATED breadth tr_rprop seed (bf16). Fused Rprop step (per-elem step sizes)."""
...
def _tr_rprop_kernel(param_ptr, grad_ptr, prev_ptr, step_size_ptr, etam, etap, smin, smax, numel, BLOCK: tl.constexpr):
    ...
    tl.store(prev_ptr + offs, prev.to(tl.bfloat16), mask=mask)
    tl.store(step_size_ptr + offs, step_size.to(tl.bfloat16), mask=mask)
