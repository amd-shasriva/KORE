"""GENERATED vendor-baselined NEOX RoPE seed (fp16) vs aiter.rope_fwd.
x[S,B,H,D], freqs[S,1,1,D//2] angles. One program per (s,b,h) row; half-width
rotate-NEOX identity (o1=x1*cos-x2*sin, o2=x2*cos+x1*sin), fp32 math, tl.float16 store.
...
def _rope_kernel(x_ptr, f_ptr, y_ptr, B, H, D,
    ...
    tl.store(y_ptr + base + (offs + HALF) * sxd, (x2 * cos + x1 * sin).to(tl.float16))
