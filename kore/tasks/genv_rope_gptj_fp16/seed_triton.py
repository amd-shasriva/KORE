"""GENERATED GPT-J (interleaved) RoPE seed (fp16).
x[S,B,H,D], freqs[S,1,1,D//2] angles. Rotates interleaved pairs (x[2i], x[2i+1])
by angle[i]: o[2i]=x1*cos-x2*sin, o[2i+1]=x2*cos+x1*sin. fp32 math, tl.float16 store.
...
def _rope_gptj_kernel(x_ptr, f_ptr, y_ptr, B, H, D,
    ...
    cos = tl.cos(theta)
    sin = tl.sin(theta)
    tl.store(y_ptr + base + (2 * offs) * sxd, (x1 * cos - x2 * sin).to(tl.float16))
    tl.store(y_ptr + base + (2 * offs + 1) * sxd, (x2 * cos + x1 * sin).to(tl.float16))
