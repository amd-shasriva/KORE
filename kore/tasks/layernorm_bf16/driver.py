"""Driver for the affine bf16 LayerNorm task (KernelForge verifier contract).

Correctness (no --bench-mode): candidate ``layernorm(x, w, b, eps)`` vs exact
torch-fp32 oracle; prints SNR / allclose / max_diff.
Bench (--bench-mode):
    --impl reference  -> AITER ``layer_norm`` (the real serving bar)
...
from kore.tasks.aiter_ref import aiter_layer_norm  # noqa: E402
...
def run_correctness(shape, mode) -> int:
    ...
    for s in seeds:
        x, w, b = ref.get_inputs(shape, device=dev, seed=s)
        r = ref.layernorm_ref(x, w, b)
        try:
            o = fn(x, w, b, ref.EPS)
...
def run_bench(shape, impl, warmup, iters) -> int:
    dev = "cuda"
    x, w, b = ref.get_inputs(shape, device=dev, seed=0)
    if impl == "reference":
        fn = lambda: aiter_layer_norm(x, w, b, ref.EPS)
    elif impl == "torch":
        fn = lambda: ref.layernorm_ref(x, w, b)
    else:
        cand = _load_candidate()
        fn = lambda: cand(x, w, b, ref.EPS)
