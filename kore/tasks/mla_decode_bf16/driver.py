"""Driver for bf16 MLA (Multi-head Latent Attention) decode.

Correctness (no --bench-mode): candidate ``mla(q_nope, q_pe, c_kv, k_pe, w_uk,
w_uv)`` vs the exact fp32 MLA oracle; SNR / allclose / max_diff (bf16 MLA gate
~25 dB; the extra latent up-projections lower fidelity slightly vs plain attention).
Bench (--bench-mode):
    --impl reference  -> vectorized torch MLA (materialize per-head K/V via batched
                         GEMM; the non-fused bar the absorbed kernel beats)
    --impl candidate  -> kernel.py

MLA is HELD OUT of training (registry HELDOUT_TASKS): this measures generalization.
"""
...
def run_correctness(shape, mode) -> int:
    ...
    for s in range(_num_correct_trials()):
        ins = ref.get_inputs(shape, device=dev, seed=s)
        r = ref.mla_ref(*ins)
        try:
            o = fn(*ins)
...
def run_bench(shape, impl, warmup, iters) -> int:
    dev = "cuda"
    ins = ref.get_inputs(shape, device=dev, seed=0)
    if impl == "reference":
        fn = lambda: ref.mla_batched(*ins)     # vectorized torch MLA bar
    elif impl == "torch":
        fn = lambda: ref.mla_ref(*ins)
    else:
