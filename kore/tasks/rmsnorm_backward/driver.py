"""Driver for the RMSNorm BACKWARD kernel (KernelForge verifier contract).

Correctness (no --bench-mode): candidate ``rmsnorm_backward(x, w, dy) -> (dx, dw)``.
  * SNR  = worst SNR over the two gradients (dx, dw) vs the torch AUTOGRAD oracle.
  * allclose = both gradients within a magnitude-scaled tolerance (bf16).
Bench (--bench-mode):
    --impl reference  -> torch autograd backward (the bar to beat)
...
def run_correctness(shape, mode) -> int:
    ...
    for s in range(_num_correct_trials()):
        x, w, dy = ref.get_inputs(shape, device=dev, seed=s)
        dx_ref, dw_ref = ref.backward_ref(x, w, dy)
        try:
            dx, dw = fn(x, w, dy)
            ...
            return 0
        torch.cuda.synchronize()
        worst = min(worst, snr_db(dx, dx_ref), snr_db(dw, dw_ref))
        maxd = max(maxd, (dx.float() - dx_ref).abs().max().item(),
                   (dw.float() - dw_ref).abs().max().item())
        dx_ok = torch.allclose(dx.float(), dx_ref.float(), atol=2e-2, rtol=2e-2)
        dw_ok = torch.allclose(dw.float(), dw_ref.float(), atol=2e-1, rtol=2e-2)
        ok = ok and dx_ok and dw_ok
...
def run_bench(shape, impl, warmup, iters) -> int:
    dev = "cuda"
    x, w, dy = ref.get_inputs(shape, device=dev, seed=0)
    if impl in ("reference", "torch"):
        def fn():
            xf = x.float().detach().requires_grad_(True)
            wf = w.float().detach().requires_grad_(True)
            rms = torch.rsqrt(xf.pow(2).mean(-1, keepdim=True) + ref.EPS)
            y = xf * rms * wf
            y.backward(dy.float())
            return xf.grad, wf.grad
    else:
        cand = _load_candidate()
        fn = lambda: cand(x, w, dy)
