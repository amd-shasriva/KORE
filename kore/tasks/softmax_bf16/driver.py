"""Driver for the bf16 row-softmax task (KernelForge verifier contract).

Correctness (no --bench-mode): candidate ``softmax(x)`` vs exact torch-fp32
oracle; prints SNR / allclose / max_diff.
Bench (--bench-mode):
    --impl reference  -> ``torch.softmax`` == fused rocm/MIOpen softmax (the real
                         framework serving bar; AITER has no dense row-softmax)
...
from kore.tasks.aiter_ref import torch_softmax_lastdim  # noqa: E402
...
def run_bench(shape, impl, warmup, iters) -> int:
    ...
    if impl == "reference":
        fn = lambda: torch_softmax_lastdim(x)
    elif impl == "torch":
        fn = lambda: ref.softmax_ref(x)
    else:
