# Specification: `gemm_bias_silu` (bf16)
...
## Inputs
- `a`: 2-D, shape `[M, K]`, dtype bf16, contiguous.
- `b`: 2-D, shape `[K, N]`, dtype bf16, contiguous.
- `bias`: 1-D, shape `[N]`, dtype bf16, contiguous.
...
2. `gemm_bias_silu` must reach at least **30 dB SNR** against the fp32 oracle on EVERY validation shape, including the one whose N is not a multiple of a power of two.
