...
is declared, that `gpu`/`release` stay excluded by default, and that no test
ever carries both `cpu` and a resource marker.
...
`test_gpu_verifier.py` and `test_gpu_timing_protocol.py` are the tests that
touch hardware directly, and they use no stubs: the driver really runs in a
subprocess through `KoreEnv._env` / `KoreEnv._exec`, the candidate really
compiles through Triton, and every verdict comes from measured output. This is
the half of `kore/env/kore_env.py` the CPU suite cannot reach:
`tests/test_phase0_verifier_fixes.py` monkeypatches `_exec`, `_env`,
`_bench_multi` and `_bench_all` away, so it tests the verifier's decision logic
and never its execution.
...
reason. When AITER is not importable, the vendor-baseline tests skip and say
so, because a pass there would hide every `aiter_*` task silently measuring the
torch framework path instead of the production kernel.
...
- the full `evaluate` path on a real card: compile, correctness on every
  requested shape, finite positive candidate and baseline timings, and the
  reward tier the observation lands in. A busy node may fail the CV/CI
  admission gates, which demotes the timing to `timing_grade="screening"`; the
  test asserts both outcomes precisely (publication implies `correct_timed` and
  a clean independent recompute; screening implies `correct_screening` with a
  stated measurement-noise reason) instead of accepting either loosely.
- a deliberately wrong kernel rejected by measured SNR, not by the static hack
  scanner and not by a mock, and never timed.
- the post-timing re-verification catching a **stateful** kernel that is
  correct when checked and wrong while timed (the invocation-count timing
  hack). A second test runs the same kernel with `do_bench=False` to show it
  does pass the correctness gate alone, so the anti-hack re-check is provably
...
- the AITER vendor baseline: whether the runtime loads at all, and whether the
  ...
  `hipblaslt_vendor` rather than a silent `framework` fallback. Tasks declaring
  a vendor bar are benched with `--impl reference` and must emit a vendor
  sentinel; a task declaring the framework bar must emit `framework`. This is
  the runtime confirmation `kore.data.schemas.resolve_baseline_identity`
  explicitly does not provide.
- `HIP_VISIBLE_DEVICES` mapping and partial-allocation detection, checked
  against device UUIDs so the ordinal itself is not taken on trust, plus
  agreement between `_gpu_selection`'s recorded provenance and the child's
  real environment.
...
- the paired protocol on live output: exactly `repeat` `KORE_TIMING_PAIR`
  lines, ascending pair indices, alternating and balanced AB/BA ordering,
  `ratio` and `log_speedup` derivable from the raw times, and acceptance by the
  verifier's own `_parse_timing_pairs`.
...
`test_gpu_e2e_serving_gate.py` is the third `gpu` module and the one exception
to "the marker means this process touches a card": it measures the end-to-end
...
[`docs/E2E_SERVING_GATE.md`](../docs/E2E_SERVING_GATE.md) has the serve
command. The engine-free half of the same path is `test_e2e_serving_gate.py`,
which runs the real HTTP client against a stdlib stub endpoint in the default
CPU suite.

### CI

`.github/workflows/gpu.yml` runs this suite, but on `workflow_dispatch` only: no
GitHub-hosted runner has an AMD GPU, and a job pinned to a self-hosted label
with no matching runner would queue until it timed out, a red PR rather than a
clean skip. Dispatched against a GPU-less runner it still stays green: a
preflight step detects the missing `/dev/kfd`/`rocm-smi` and turns the
remaining steps off, and the suite itself would skip every test anyway. On a
Slurm cluster, `scripts/spur_gpu_smoke.sbatch` is the batch entry point.
...
| Evolutionary search | `test_evolve_agent.py` (strategy-niche diversity metric versus the speedup-binned archive, reward hacking, lazy optimisation, population collapse, noise-driven collapse, best-of-history scaling) |
...
| Frontier data selection | `test_frontier_selection.py`, `test_frontier_pipeline_contract.py` (family classification, the orderings that make a win valuable, and the launch-bound penalty) |
| AgentKernelArena | `test_agent_kernel_arena.py`, `test_arena_attempts.py`, `test_arena_bench_serialization.py`, `test_arena_code_extraction.py`, `test_arena_extension_contract.py`, `test_arena_purge_harness_failures.py`, `test_arena_pytest_isolation.py`, `test_arena_repo_staging.py`, `test_arena_speedup_parsing.py`, `test_arena_splice.py`, `test_arena_task_env.py`, `test_arena_api_arm.py`, `test_arena_wall_clock.py`, `test_arena_never_starved.py`, `test_arena_baseline_gate.py`. Every one of these pins a defect that scored a whole category zero without looking like a failure: a swallowed speedup, a spliced-away import, our own pytest config deselecting a task's GPU tests, an interpreter without torch, concurrent benchmarking biasing every measurement down, and a target file that ships empty with its extension contract stated nowhere. `test_arena_baseline_gate.py` is the newest and the bluntest: a sweep that runs without a baseline pass cannot produce a speedup for most of the arena, and the loss is unrecoverable because workspaces are deleted after scoring, so the run now refuses to start rather than warning |
...
| Stage readiness / resume | `test_sft_launch_readiness.py`, `test_sft_readiness.py`, `test_dpo_launch_readiness.py`, `test_grpo_launch_readiness.py`, `test_grpo_capabilities.py`, `test_grpo_checkpoint_resume.py`, `test_minimum_grpo_profile.py`, `test_preemption_checkpoints.py`, `test_rl_readiness_fixes.py` |
...
`tests/test_*.py` exists that this table does not list, because the useful
half of this suite is the contract tests, and an unlisted contract test is one
nobody knows to look at when the thing it pins changes.
...
actually works on hardware. Resource-specific tests belong to an explicit
marker group; the default remains CPU-only.

## Known limitations

* This environment has no `torch` installed, so any test module that imports
  torch at collection time (most of `kore/**/tests` and several `tests/*.py`
  modules) cannot even be collected here. That is an environment gap, not a
  code defect; verify on a node with the full training environment installed.
* The coverage map above is a manually maintained index over an automatically
  enforced set (`test_docs_contract.py` checks completeness both ways). Adding
  a test file without also adding it to the right row here is a documentation
  defect the contract test will catch on the next CI run, not silently.

See also: [`kore/verify`](../kore/verify/README.md),
[`kore/reward`](../kore/reward/README.md), [`kore/tasks`](../kore/tasks/README.md).
