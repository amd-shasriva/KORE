...
class _MarkerEnv:
    ...
    def step(self, source, full_validation=True, multi_shape=True):
        ...
        baseline_ms = 1.0
        obs = Observation(compiled=True, validation_passed=correct, snr_db=snr,
                          snr_by_shape={"s": snr},
                          wall_ms=wall_ms,
                          baseline_ms=baseline_ms, dtype="bf16")
        # New baseline-relative + variance-gated admission: emit the per-shape timing
        # classification + tight CVs a real verifier produces so a faster kernel admits.
